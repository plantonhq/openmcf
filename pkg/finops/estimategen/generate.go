...
import (
	...
	"github.com/plantonhq/planton/pkg/finops/capacityderivation"
...
// modelHeader opens estimate files generated from a hand-authored
// estimate model. It names the regeneration command and both input
// documents so a reader lands on the authoring surfaces, never on the
// generated output.
const modelHeader = `# GENERATED -- DO NOT EDIT. Regenerate with ` + "`make generate-cost-estimates`" + `.
...
// derivationHeader opens estimate files generated by replaying the
// component's presets through its cost derivation -- the same rules a
// server-side estimator evaluates against any manifest, held to these
// committed numbers by the drift gate.
const derivationHeader = `# GENERATED -- DO NOT EDIT. Regenerate with ` + "`make generate-cost-estimates`" + `.
...
// capacityHeader opens estimate files generated by replaying the
// component's presets through its capacity derivation. Cluster-capacity
// components price nothing -- their estimates state the cluster resources
// each preset reserves, with the same drift gate holding the rules to
// these committed footprints.
const capacityHeader = `# GENERATED -- DO NOT EDIT. Regenerate with ` + "`make generate-cost-estimates`" + `.
# This file replays the component's presets through its capacity derivation
# (../capacity/, the machine-executable value-to-capacity rules): each
# preset states the CPU/memory requests and limits and the persistent
# volume storage it reserves from its target cluster, with a basis sentence
# tracing every total back to the manifest's values. Cluster-capacity
# components carry no priced lines -- the dollar value of the reservation
# is the target cluster's node and storage-class economics, which this
# catalog cannot know. Schema: finops/componentcostestimate/v1.
`
...
func Generate(repoRoot string) (*Summary, error) {
	...
	modeled, err := estimatemodel.Discover(repoRoot)
	if err != nil {
		return nil, err
	}
	derived, err := costderivation.Discover(repoRoot)
	if err != nil {
		return nil, err
	}
	derivedSet := map[string]bool{}
	for _, component := range derived {
		derivedSet[component] = true
	}
	capacity, err := capacityderivation.Discover(repoRoot)
	if err != nil {
		return nil, err
	}
	capacitySet := map[string]bool{}
	for _, component := range capacity {
		capacitySet[component] = true
	}
	components := append(append(append([]string{}, modeled...), derived...), capacity...)
	...
	for _, component := range components {
		...
		// Quantities have exactly one home: a hand-authored model OR a
		// machine-executable derivation (cost or capacity), never both.
		header := modelHeader
		var model *estimatemodelv1.ComponentCostEstimateModel
		if capacitySet[component] {
			if _, err := estimatemodel.Load(repoRoot, component); err == nil {
				summary.problemf("%s: both an estimate model and a capacity derivation exist -- quantities have exactly one home; delete the model", component)
				continue
			}
			if derivedSet[component] {
				summary.problemf("%s: both a cost derivation and a capacity derivation exist -- a cluster-capacity component prices no meters; delete one", component)
				continue
			}
			header = capacityHeader
			model = replayCapacityPresets(summary, repoRoot, provider, component)
			if model == nil {
				continue
			}
		} else if derivedSet[component] {
			if _, err := estimatemodel.Load(repoRoot, component); err == nil {
				summary.problemf("%s: both an estimate model and a cost derivation exist -- quantities have exactly one home; delete the model", component)
				continue
			}
			header = derivationHeader
			model = replayPresets(summary, repoRoot, provider, component, entries)
			if model == nil {
				continue
			}
		...
		doc, ok := renderComponent(summary, header, component, model, entries, meterUnits, meterService, referenced[provider])
		if ok {
...
// replayPresets evaluates a component's cost derivation against every
// catalog preset, producing the same shape a hand-authored model carries.
// A preset the derivation refuses is honestly absent; an evaluation error
// is a coherence problem (the derivation gate should have caught it).
// Returns nil when nothing can be replayed.
func replayPresets(summary *Summary, repoRoot, provider, component string, entries map[string]*pricebookv1.PriceBookEntry) *estimatemodelv1.ComponentCostEstimateModel {
	derivation, err := costderivation.Load(repoRoot, component)
	if err != nil {
		summary.problemf("%s: %v", component, err)
		return nil
	}

	matches, err := filepath.Glob(filepath.Join(repoRoot, "catalog", provider, component, "presets", "*.yaml"))
	if err != nil {
		summary.problemf("%s: globbing presets: %v", component, err)
		return nil
	}
	sort.Strings(matches)
	if len(matches) == 0 {
		summary.problemf("%s: a cost derivation exists but the component ships no presets to replay", component)
		return nil
	}

	kind := crkreflect.KindFromString(component)
	model := &estimatemodelv1.ComponentCostEstimateModel{Spec: &estimatemodelv1.ComponentCostEstimateModelSpec{}}
	for _, presetPath := range matches {
		stem := strings.TrimSuffix(filepath.Base(presetPath), ".yaml")
		manifest, err := crkreflect.NewInstance(kind)
		if err != nil {
			summary.problemf("%s: resolving kind: %v", component, err)
			return nil
		}
		if err := protobufyaml.Load(presetPath, manifest); err != nil {
			summary.problemf("%s preset %s: %v", component, stem, err)
			continue
		}
		...
		if refusal != nil {
			// The honest omission: this preset's committed cost is
			// unknowable from its manifest, so it gets no estimate entry.
			continue
		}
		preset.Preset = stem
		model.Spec.Presets = append(model.Spec.Presets, preset)
	}
	if len(model.GetSpec().GetPresets()) == 0 {
		summary.problemf("%s: the derivation refused every preset -- a component whose presets never price should ship no derivation", component)
		return nil
	}
	return model
}

// replayCapacityPresets evaluates a cluster-capacity component's capacity
// derivation against every catalog preset, producing the same shape a
// hand-authored model carries (capacity footprints, no priced lines). A
// preset the derivation refuses (no reservation stated in the manifest)
// is honestly absent. Returns nil when nothing can be replayed.
func replayCapacityPresets(summary *Summary, repoRoot, provider, component string) *estimatemodelv1.ComponentCostEstimateModel {
	derivation, err := capacityderivation.Load(repoRoot, component)
	if err != nil {
		summary.problemf("%s: %v", component, err)
		return nil
	}
	...
	sort.Strings(matches)
	if len(matches) == 0 {
		summary.problemf("%s: a capacity derivation exists but the component ships no presets to replay", component)
		return nil
	}
	...
	for _, presetPath := range matches {
		...
		preset, refusal, err := capacityestimator.Evaluate(manifest, derivation.GetSpec())
		if err != nil {
			summary.problemf("%s preset %s: %v", component, stem, err)
			continue
		}
		if refusal != nil {
			// The honest omission: this preset states no reservation the
			// rules can read, so it gets no estimate entry.
			continue
		}
		preset.Preset = stem
		model.Spec.Presets = append(model.Spec.Presets, preset)
	...
	if len(model.GetSpec().GetPresets()) == 0 {
		summary.problemf("%s: the capacity derivation refused every preset -- a component whose presets state no reservations should ship no capacity derivation", component)
		return nil
	}
	return model
}
