#!/usr/bin/env python3
"""
Deterministic tool: Write outputs.proto and run the proto build.

Usage:
  cat outputs.proto | python3 _rules/deployment-component/_scripts/outputs_write_and_build.py --provider aws --kindfolder awscloudfront --stdin
...
def outputs_path(repo_root: str, provider: str, kind_folder: str) -> Tuple[str, str]:
    rel = os.path.join(
        "catalog", provider, kind_folder, "v1alpha1", "outputs.proto")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write outputs.proto and build")
