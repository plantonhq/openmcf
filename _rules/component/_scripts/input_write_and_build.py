...
def input_proto_path(repo_root: str, provider: str, kind_folder: str) -> Tuple[str, str]:
    rel = os.path.join(
        "catalog", provider, kind_folder, "v1alpha1", "input.proto")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Write input.proto and run the proto build")
    ...
    abs_path, rel_path = input_proto_path(repo_root, provider, kind)
