# Provider requirements for the KubernetesListenerSet module.
#
# kubectl (alekc/kubectl) applies the ListenerSet CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the ListenerSet can be planned before the
