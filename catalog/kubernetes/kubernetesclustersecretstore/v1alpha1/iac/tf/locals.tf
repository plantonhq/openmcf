# Computed values for the KubernetesClusterSecretStore module.
#
...
# Pulumi modules use — and the KubernetesSecretStore Terraform module
# carries the same rendering with a namespace-scoped kind. Keep all of them
# in lockstep: same CRD field names, same credential Secret name, same
# secret data keys.
#
...
# the CR's secretRefs point at it. Cluster-scoped stores read referenced
# Secrets from an EXPLICIT namespace, so every ref carries the secrets
# namespace (the namespaced twin omits it).
#
# Optional nested blocks are read with try(): HCL's && does NOT
# short-circuit, so chained null checks still dereference the null.

locals {
  # ClusterSecretStore metadata.name — the name ExternalSecrets reference
  # (kind ClusterSecretStore).
  store_name = var.metadata.name

  # Namespace where declared credential Secrets materialize —
  # conventionally the operator's install namespace (resolved literal from
  # the spec's value-or-ref).
  secrets_namespace = var.spec.secrets_namespace

  labels = merge(concat(
    [{
      "planton.ai/resource"      = "true"
      "planton.ai/resource-name" = var.metadata.name
      "planton.ai/resource-kind" = "KubernetesClusterSecretStore"
    }],
  ...
  # addresses the one credential Secret by fixed key. Cluster-scoped
  # stores carry the explicit secrets namespace on each ref.
  secret_ref = {
    for key in [
      "access-key-id",
      "secret-access-key",
      ...
      "bearer-token",
    ] : key => { name = local.credential_secret_name, key = key, namespace = local.secrets_namespace }
  }
      ...
      auth = (try(local.aws.access_key_id, "") != "" || try(local.aws.service_account_name, "") != "") ? {
        for ak, av in {
          ...
          jwt = (try(local.aws.access_key_id, "") == "" && try(local.aws.service_account_name, "") != "") ? {
            # On a ClusterSecretStore the serviceAccountRef namespace is
            # REQUIRED by the webhook (cluster scope has no home
            # namespace): the per-backend override when set, else the
            # secrets namespace.
            serviceAccountRef = {
              name      = local.aws.service_account_name
              namespace = try(local.aws.service_account_namespace, "") != "" ? local.aws.service_account_namespace : local.secrets_namespace
            }
          } : null
        } : ak => av if av != null
      } : null
        ...
        for ak, av in {
          ...
          workloadIdentity = (try(local.gcp.service_account_key_json, "") == "" && try(local.gcp.service_account_name, "") != "") ? {
            serviceAccountRef = {
              ...
              namespace = try(local.gcp.service_account_namespace, "") != "" ? local.gcp.service_account_namespace : local.secrets_namespace
            }
          } : null
        } : ak => av if av != null
      } : null
  ...
  provider_azure = local.azure == null ? null : {
    for k, v in {
      ...
      serviceAccountRef = try(local.azure.service_account_name, "") != "" ? {
        ...
        namespace = try(local.azure.service_account_namespace, "") != "" ? local.azure.service_account_namespace : local.secrets_namespace
      } : null
        ...
        for ak, av in {
          ...
          kubernetes = try(local.vault.kubernetes, null) == null ? null : {
            for kk, kv in {
              ...
              # The Go builder passes no per-backend namespace here, so a
              # cluster-scoped store stamps the secrets namespace.
  ...
  # ---- namespace fence ----------------------------------------------------
  # The cluster kind's conditions: which namespaces' ExternalSecrets may
  # sync from this store. Twin of the Pulumi module's buildConditions —
  # entries selecting nothing are skipped, matching its len(entry) guard.
  store_conditions = [
    for c in try(var.spec.conditions, []) : {
      for k, v in {
        namespaces        = length(try(c.namespaces, [])) > 0 ? c.namespaces : null
        namespaceSelector = length(try(c.namespace_label_selector, {})) > 0 ? { matchLabels = c.namespace_label_selector } : null
        namespaceRegexes  = length(try(c.namespace_regexes, [])) > 0 ? c.namespace_regexes : null
      } : k => v if v != null
    }
    if length(try(c.namespaces, [])) > 0 || length(try(c.namespace_label_selector, {})) > 0 || length(try(c.namespace_regexes, [])) > 0
  ]
  ...
  store_spec = {
    for k, v in {
      ...
      conditions      = length(local.store_conditions) > 0 ? local.store_conditions : null
