...
variable "spec" {
  description = "KubernetesStatefulSet specification"
  type = object({
    namespace = string
    ...
    pvc_retention_policy = optional(object({
      when_deleted = optional(string, "")
      when_scaled = optional(string, "")
    }))
    ordinals = optional(object({
