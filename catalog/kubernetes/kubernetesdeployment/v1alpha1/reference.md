...
- `AwsLambdaLayer` -- A Lambda layer version - a shared code archive (libraries, custom runtimes) functions attach by ARN - with its cross-account and organization share grants managed in-line. The archive lives in S3 (an optional reference, so no registry prerequisite - lanes compose their own bucket fixture). 1351 sits in the app & data services sub-band (1350-1359; 1350 is reserved for AwsAppSyncApi).
