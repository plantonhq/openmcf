...
- `AwsOrganization` -- THE AWS Organization of the deploying account - creating it makes the caller the management account. Trusted service access, delegated administrators, and the org's singleton resource policy fold in (none has a life of its own; the standalone service-access resource fights the org's own argument with a perpetual diff). Deleting this deletes the entire organization. 1270 opens the Organizations sub-band (1270-1279).
