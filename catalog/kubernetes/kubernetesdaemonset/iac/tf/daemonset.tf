# The apps/v1 DaemonSet. Every container (app and sidecars) renders through
...
# Only DaemonSet-specific mechanics (node-by-node rollout) live at the top of
# the spec; which nodes run the agent is controlled entirely through the pod's
# scheduling rules (node_selector, node_affinity, tolerations).
#
# Identity and permissions are composed, not created: pods run as the
# ServiceAccount named in spec.pod.service_account, and API permissions come
# from KubernetesRbac grants targeting that identity. This module never
...
resource "kubernetes_daemon_set_v1" "this" {
  ...
  spec {
    ...
    min_ready_seconds      = try(var.spec.min_ready_seconds, null)
    revision_history_limit = try(var.spec.revision_history_limit, null)

    # The provider names this block `strategy` (the API field is
    # updateStrategy); rolling_update carries both bounds — max_unavailable
    # (nodes with the agent down) and max_surge (nodes running old and new
    # side by side for gapless handoff).
    dynamic "strategy" {
      ...
      spec {
        ...
        dynamic "container" {
          ...
          content {
            ...
            # host_port is the DaemonSet exposure mechanism — agents publish
            # node-local endpoints on the node's own IP (there is no Service).
            dynamic "port" {
