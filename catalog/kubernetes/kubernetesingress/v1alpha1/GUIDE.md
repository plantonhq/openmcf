...
- **Gateway API** (KubernetesGateway + route kinds) — the right lane when
  ...
  anchor: [KubernetesGateway guide](../../kubernetesgateway/v1alpha1/GUIDE.md).
...
1. A controller claiming the `ingressClassName`
   ([KubernetesIngressNginx guide](../../kubernetesingressnginx/v1alpha1/GUIDE.md)) —
...
3. TLS: the `tls` block names a Secret that
   [cert-manager issues](../../kubernetesclusterissuer/v1alpha1/GUIDE.md) via
...
4. DNS: the hostname becomes real through
   [KubernetesExternalDns](../../kubernetesexternaldns/v1alpha1/GUIDE.md).
