...
# every sidecar, and every init container (all read from spec.job_template),
# materialized into ONE Secret (locals.env_secret_data). Secrets referenced
# from existing Kubernetes Secrets are wired directly as env references and
