# KubernetesServiceEntry Pulumi Module

This Pulumi module creates a namespaced Istio `ServiceEntry` on a target cluster using
...
- A running Istio control plane (istiod) to program the registry
  (see the `KubernetesIstio` component). The CR applies successfully with only the
  CRDs present; routing requires istiod.
...
The entrypoint loads the `KubernetesServiceEntryStackInput` from the
...
└── module/
    ├── main.go          # Resource creation (typed NewServiceEntry)
