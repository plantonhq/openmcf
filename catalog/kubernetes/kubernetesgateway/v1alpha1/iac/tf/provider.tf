# Provider requirements for the KubernetesGateway module.
#
# kubectl (alekc/kubectl) applies the Gateway CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the Gateway can be planned before the
