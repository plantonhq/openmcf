...
// chartVersionAtLeast reports whether version (an exact "x.y.z" semver)
// is at least min. Twin of the Terraform module's lifecycle precondition —
// HCL has no semver function, so both sides hand-compare the three parts.
func chartVersionAtLeast(version, min string) (bool, error) {
	v, err := semverParts(version)
	if err != nil {
		return false, err
	}
	m, err := semverParts(min)
	if err != nil {
		return false, err
	}
	for i := 0; i < 3; i++ {
...
// semverParts splits an exact "x.y.z" version into its numeric parts.
func semverParts(version string) ([3]int, error) {
	var out [3]int
	parts := strings.Split(version, ".")
	if len(parts) != 3 {
		return out, errors.Errorf("expected an exact x.y.z semver, got %q", version)
	}
	for i, p := range parts {
		n, err := strconv.Atoi(p)
		if err != nil {
			return out, errors.Wrapf(err, "non-numeric semver part %q in %q", p, version)
		}
		out[i] = n
	}
	return out, nil
