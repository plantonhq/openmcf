...
**Verdict:** Industry standard. Planton wraps this with a unified API and FK-based dependency management.
...
**Verdict:** Excellent for engineering teams. Planton uses Pulumi internally for Go-native IaC modules.
...
The Terraform `openstack_lb_loadbalancer_v2` resource exposes 14 schema fields. Planton's `OpenStackLoadBalancerSpec` exposes 7 fields -- the ones that cover 95%+ of real-world use cases:
...
**`flavor_id` as a plain string:** Flavors are typically pre-created by cloud operators. Since they're not managed by Planton, a plain string (not StringValueOrRef) is appropriate.

**Single-resource component:** Unlike some frameworks that create the entire LB hierarchy at once, Planton keeps each level as a separate component. This enables:
