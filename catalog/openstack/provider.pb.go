// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: catalog/openstack/provider.proto

package openstack

import (
	_ "buf.build/gen/go/bufbuild/protovalidate/protocolbuffers/go/buf/validate"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// OpenStackProviderConfig defines the credentials and connection parameters required to interact
// with an OpenStack cloud. This message supports multiple authentication methods via the
// `credentials` oneof, reflecting OpenStack's flexible identity model (Keystone).
//
// The three supported authentication methods are:
// - Password: Username + password with project/domain context (most common for interactive use)
// - Application Credentials: Pre-scoped ID + secret (recommended for automation and CI/CD)
// - Token: Pre-authenticated token (useful for short-lived or delegated access)
//
// Common fields (auth_url, region, project/domain context, TLS) apply to all methods.
// The Terraform OpenStack provider environment variables (OS_*) are derived from these fields.
//
// For more information:
// - Terraform provider docs: https://registry.terraform.io/providers/terraform-provider-openstack/openstack/latest/docs
// - OpenStack authentication: https://docs.openstack.org/keystone/latest/user/application_credentials.html
type OpenStackProviderConfig struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// auth_url is the Identity (Keystone) authentication endpoint URL.
	// This is the entry point for all OpenStack API authentication.
	// Typically ends with /v3 for Identity API v3.
	// Example: "https://cloud.example.com:5000/v3"
	// Environment variable: OS_AUTH_URL
	AuthUrl string `protobuf:"bytes,1,opt,name=auth_url,json=authUrl,proto3" json:"auth_url,omitempty"`
	// region is the OpenStack region to operate in.
	// Regions represent separate geographic locations within a single OpenStack deployment.
	// If your cloud has only one region, this can often be omitted.
	// Example: "RegionOne"
	// Environment variable: OS_REGION_NAME
	Region string `protobuf:"bytes,2,opt,name=region,proto3" json:"region,omitempty"`
	// credentials specifies the authentication method and its associated fields.
	// Exactly one method must be provided.
	//
	// Types that are valid to be assigned to Credentials:
	//
	//	*OpenStackProviderConfig_Password
	//	*OpenStackProviderConfig_ApplicationCredential
	//	*OpenStackProviderConfig_Token
	Credentials isOpenStackProviderConfig_Credentials `protobuf_oneof:"credentials"`
	// tenant_name is the name of the OpenStack project (tenant) to scope operations to.
	// Required for password and token authentication; not needed for application credentials.
	// Also known as project_name.
	// Environment variable: OS_TENANT_NAME / OS_PROJECT_NAME
	TenantName string `protobuf:"bytes,20,opt,name=tenant_name,json=tenantName,proto3" json:"tenant_name,omitempty"`
	// tenant_id is the unique ID of the OpenStack project (tenant).
	// Alternative to tenant_name. Use one or the other, not both.
	// Environment variable: OS_TENANT_ID / OS_PROJECT_ID
	TenantId string `protobuf:"bytes,21,opt,name=tenant_id,json=tenantId,proto3" json:"tenant_id,omitempty"`
	// user_domain_name is the domain name of the authenticating user.
	// Required for Identity v3 password authentication when the user is not in the default domain.
	// Common value: "Default"
	// Environment variable: OS_USER_DOMAIN_NAME
	UserDomainName string `protobuf:"bytes,30,opt,name=user_domain_name,json=userDomainName,proto3" json:"user_domain_name,omitempty"`
	// user_domain_id is the domain ID of the authenticating user.
	// Alternative to user_domain_name.
	// Environment variable: OS_USER_DOMAIN_ID
	UserDomainId string `protobuf:"bytes,31,opt,name=user_domain_id,json=userDomainId,proto3" json:"user_domain_id,omitempty"`
	// project_domain_name is the domain name of the target project.
	// Required for Identity v3 when the project is not in the default domain.
	// Common value: "Default"
	// Environment variable: OS_PROJECT_DOMAIN_NAME
	ProjectDomainName string `protobuf:"bytes,32,opt,name=project_domain_name,json=projectDomainName,proto3" json:"project_domain_name,omitempty"`
	// project_domain_id is the domain ID of the target project.
	// Alternative to project_domain_name.
	// Environment variable: OS_PROJECT_DOMAIN_ID
	ProjectDomainId string `protobuf:"bytes,33,opt,name=project_domain_id,json=projectDomainId,proto3" json:"project_domain_id,omitempty"`
	// insecure controls whether to trust self-signed or invalid TLS certificates.
	// Set to true only for development/testing environments.
	// Default: false
	// Environment variable: OS_INSECURE
	Insecure bool `protobuf:"varint,40,opt,name=insecure,proto3" json:"insecure,omitempty"`
	// cacert_file is the path to a custom CA certificate bundle for TLS verification.
	// Use this when your OpenStack deployment uses a private or corporate CA.
	// Environment variable: OS_CACERT
	CacertFile string `protobuf:"bytes,41,opt,name=cacert_file,json=cacertFile,proto3" json:"cacert_file,omitempty"`
	// endpoint_type specifies which endpoint type to use from the service catalog.
	// Valid values: "public" (default), "internal", "admin"
	// Environment variable: OS_ENDPOINT_TYPE
	EndpointType  string `protobuf:"bytes,50,opt,name=endpoint_type,json=endpointType,proto3" json:"endpoint_type,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OpenStackProviderConfig) Reset() {
	*x = OpenStackProviderConfig{}
	mi := &file_catalog_openstack_provider_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OpenStackProviderConfig) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OpenStackProviderConfig) ProtoMessage() {}

func (x *OpenStackProviderConfig) ProtoReflect() protoreflect.Message {
	mi := &file_catalog_openstack_provider_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OpenStackProviderConfig.ProtoReflect.Descriptor instead.
func (*OpenStackProviderConfig) Descriptor() ([]byte, []int) {
	return file_catalog_openstack_provider_proto_rawDescGZIP(), []int{0}
}

func (x *OpenStackProviderConfig) GetAuthUrl() string {
	if x != nil {
		return x.AuthUrl
	}
	return ""
}

func (x *OpenStackProviderConfig) GetRegion() string {
	if x != nil {
		return x.Region
	}
	return ""
}

func (x *OpenStackProviderConfig) GetCredentials() isOpenStackProviderConfig_Credentials {
	if x != nil {
		return x.Credentials
	}
	return nil
}

func (x *OpenStackProviderConfig) GetPassword() *OpenStackPasswordCredentials {
	if x != nil {
		if x, ok := x.Credentials.(*OpenStackProviderConfig_Password); ok {
			return x.Password
		}
	}
	return nil
}

func (x *OpenStackProviderConfig) GetApplicationCredential() *OpenStackApplicationCredentials {
	if x != nil {
		if x, ok := x.Credentials.(*OpenStackProviderConfig_ApplicationCredential); ok {
			return x.ApplicationCredential
		}
	}
	return nil
}

func (x *OpenStackProviderConfig) GetToken() *OpenStackTokenCredentials {
	if x != nil {
		if x, ok := x.Credentials.(*OpenStackProviderConfig_Token); ok {
			return x.Token
		}
	}
	return nil
}

func (x *OpenStackProviderConfig) GetTenantName() string {
	if x != nil {
		return x.TenantName
	}
	return ""
}

func (x *OpenStackProviderConfig) GetTenantId() string {
	if x != nil {
		return x.TenantId
	}
	return ""
}

func (x *OpenStackProviderConfig) GetUserDomainName() string {
	if x != nil {
		return x.UserDomainName
	}
	return ""
}

func (x *OpenStackProviderConfig) GetUserDomainId() string {
	if x != nil {
		return x.UserDomainId
	}
	return ""
}

func (x *OpenStackProviderConfig) GetProjectDomainName() string {
	if x != nil {
		return x.ProjectDomainName
	}
	return ""
}

func (x *OpenStackProviderConfig) GetProjectDomainId() string {
	if x != nil {
		return x.ProjectDomainId
	}
	return ""
}

func (x *OpenStackProviderConfig) GetInsecure() bool {
	if x != nil {
		return x.Insecure
	}
	return false
}

func (x *OpenStackProviderConfig) GetCacertFile() string {
	if x != nil {
		return x.CacertFile
	}
	return ""
}

func (x *OpenStackProviderConfig) GetEndpointType() string {
	if x != nil {
		return x.EndpointType
	}
	return ""
}

type isOpenStackProviderConfig_Credentials interface {
	isOpenStackProviderConfig_Credentials()
}

type OpenStackProviderConfig_Password struct {
	// password authenticates using a username and password.
	// This is the most common method for interactive use and requires
	// project/domain context fields to be set.
	Password *OpenStackPasswordCredentials `protobuf:"bytes,10,opt,name=password,proto3,oneof"`
}

type OpenStackProviderConfig_ApplicationCredential struct {
	// application_credential authenticates using pre-scoped application credentials.
	// This is the recommended method for automation and CI/CD pipelines.
	// Application credentials are scoped to a specific project and set of roles,
	// eliminating the need for separate project/domain context fields.
	ApplicationCredential *OpenStackApplicationCredentials `protobuf:"bytes,11,opt,name=application_credential,json=applicationCredential,proto3,oneof"`
}

type OpenStackProviderConfig_Token struct {
	// token authenticates using a pre-existing Keystone token.
	// Useful for short-lived access or when delegating from another service.
	Token *OpenStackTokenCredentials `protobuf:"bytes,12,opt,name=token,proto3,oneof"`
}

func (*OpenStackProviderConfig_Password) isOpenStackProviderConfig_Credentials() {}

func (*OpenStackProviderConfig_ApplicationCredential) isOpenStackProviderConfig_Credentials() {}

func (*OpenStackProviderConfig_Token) isOpenStackProviderConfig_Credentials() {}

// OpenStackPasswordCredentials provides username/password authentication.
// This method authenticates directly with Keystone using user credentials.
// It requires project/domain context to be set on the parent OpenStackProviderConfig
// so that the resulting token is scoped to the correct project.
type OpenStackPasswordCredentials struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// user_name is the OpenStack username.
	// Environment variable: OS_USERNAME
	UserName string `protobuf:"bytes,1,opt,name=user_name,json=userName,proto3" json:"user_name,omitempty"`
	// password is the user's password.
	// Environment variable: OS_PASSWORD
	Password      string `protobuf:"bytes,2,opt,name=password,proto3" json:"password,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OpenStackPasswordCredentials) Reset() {
	*x = OpenStackPasswordCredentials{}
	mi := &file_catalog_openstack_provider_proto_msgTypes[1]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OpenStackPasswordCredentials) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OpenStackPasswordCredentials) ProtoMessage() {}

func (x *OpenStackPasswordCredentials) ProtoReflect() protoreflect.Message {
	mi := &file_catalog_openstack_provider_proto_msgTypes[1]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OpenStackPasswordCredentials.ProtoReflect.Descriptor instead.
func (*OpenStackPasswordCredentials) Descriptor() ([]byte, []int) {
	return file_catalog_openstack_provider_proto_rawDescGZIP(), []int{1}
}

func (x *OpenStackPasswordCredentials) GetUserName() string {
	if x != nil {
		return x.UserName
	}
	return ""
}

func (x *OpenStackPasswordCredentials) GetPassword() string {
	if x != nil {
		return x.Password
	}
	return ""
}

// OpenStackApplicationCredentials provides authentication via application credentials.
// Application credentials are pre-scoped to a specific project and set of roles,
// making them ideal for automation. They do not require separate project/domain context.
//
// Either id or name must be provided (along with secret).
// Using id is recommended as it is globally unique; name requires the parent user context.
type OpenStackApplicationCredentials struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// id is the unique identifier of the application credential.
	// Preferred over name as it is globally unique.
	// Environment variable: OS_APPLICATION_CREDENTIAL_ID
	Id string `protobuf:"bytes,1,opt,name=id,proto3" json:"id,omitempty"`
	// name is the human-readable name of the application credential.
	// If using name instead of id, the user context must also be determinable.
	// Environment variable: OS_APPLICATION_CREDENTIAL_NAME
	Name string `protobuf:"bytes,2,opt,name=name,proto3" json:"name,omitempty"`
	// secret is the application credential secret.
	// This is a required field and must be kept secure.
	// Environment variable: OS_APPLICATION_CREDENTIAL_SECRET
	Secret        string `protobuf:"bytes,3,opt,name=secret,proto3" json:"secret,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OpenStackApplicationCredentials) Reset() {
	*x = OpenStackApplicationCredentials{}
	mi := &file_catalog_openstack_provider_proto_msgTypes[2]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OpenStackApplicationCredentials) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OpenStackApplicationCredentials) ProtoMessage() {}

func (x *OpenStackApplicationCredentials) ProtoReflect() protoreflect.Message {
	mi := &file_catalog_openstack_provider_proto_msgTypes[2]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OpenStackApplicationCredentials.ProtoReflect.Descriptor instead.
func (*OpenStackApplicationCredentials) Descriptor() ([]byte, []int) {
	return file_catalog_openstack_provider_proto_rawDescGZIP(), []int{2}
}

func (x *OpenStackApplicationCredentials) GetId() string {
	if x != nil {
		return x.Id
	}
	return ""
}

func (x *OpenStackApplicationCredentials) GetName() string {
	if x != nil {
		return x.Name
	}
	return ""
}

func (x *OpenStackApplicationCredentials) GetSecret() string {
	if x != nil {
		return x.Secret
	}
	return ""
}

// OpenStackTokenCredentials provides authentication using a pre-existing Keystone token.
// Tokens are short-lived and are typically obtained from a prior authentication step.
// Useful for delegated access patterns or when integrating with external auth systems.
type OpenStackTokenCredentials struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// token is the pre-authenticated Keystone token string.
	// Environment variable: OS_TOKEN
	Token         string `protobuf:"bytes,1,opt,name=token,proto3" json:"token,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *OpenStackTokenCredentials) Reset() {
	*x = OpenStackTokenCredentials{}
	mi := &file_catalog_openstack_provider_proto_msgTypes[3]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *OpenStackTokenCredentials) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*OpenStackTokenCredentials) ProtoMessage() {}

func (x *OpenStackTokenCredentials) ProtoReflect() protoreflect.Message {
	mi := &file_catalog_openstack_provider_proto_msgTypes[3]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use OpenStackTokenCredentials.ProtoReflect.Descriptor instead.
func (*OpenStackTokenCredentials) Descriptor() ([]byte, []int) {
	return file_catalog_openstack_provider_proto_rawDescGZIP(), []int{3}
}

func (x *OpenStackTokenCredentials) GetToken() string {
	if x != nil {
		return x.Token
	}
	return ""
}

var File_catalog_openstack_provider_proto protoreflect.FileDescriptor

const file_catalog_openstack_provider_proto_rawDesc = "" +
	"\n" +
	" catalog/openstack/provider.proto\x12\x15dev.planton.openstack\x1a\x1bbuf/validate/validate.proto\"\xbd\x05\n" +
	"\x17OpenStackProviderConfig\x12!\n" +
	"\bauth_url\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\aauthUrl\x12\x16\n" +
	"\x06region\x18\x02 \x01(\tR\x06region\x12Q\n" +
	"\bpassword\x18\n" +
	" \x01(\v23.dev.planton.openstack.OpenStackPasswordCredentialsH\x00R\bpassword\x12o\n" +
	"\x16application_credential\x18\v \x01(\v26.dev.planton.openstack.OpenStackApplicationCredentialsH\x00R\x15applicationCredential\x12H\n" +
	"\x05token\x18\f \x01(\v20.dev.planton.openstack.OpenStackTokenCredentialsH\x00R\x05token\x12\x1f\n" +
	"\vtenant_name\x18\x14 \x01(\tR\n" +
	"tenantName\x12\x1b\n" +
	"\ttenant_id\x18\x15 \x01(\tR\btenantId\x12(\n" +
	"\x10user_domain_name\x18\x1e \x01(\tR\x0euserDomainName\x12$\n" +
	"\x0euser_domain_id\x18\x1f \x01(\tR\fuserDomainId\x12.\n" +
	"\x13project_domain_name\x18  \x01(\tR\x11projectDomainName\x12*\n" +
	"\x11project_domain_id\x18! \x01(\tR\x0fprojectDomainId\x12\x1a\n" +
	"\binsecure\x18( \x01(\bR\binsecure\x12\x1f\n" +
	"\vcacert_file\x18) \x01(\tR\n" +
	"cacertFile\x12#\n" +
	"\rendpoint_type\x182 \x01(\tR\fendpointTypeB\r\n" +
	"\vcredentials\"g\n" +
	"\x1cOpenStackPasswordCredentials\x12#\n" +
	"\tuser_name\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\buserName\x12\"\n" +
	"\bpassword\x18\x02 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\bpassword\"e\n" +
	"\x1fOpenStackApplicationCredentials\x12\x0e\n" +
	"\x02id\x18\x01 \x01(\tR\x02id\x12\x12\n" +
	"\x04name\x18\x02 \x01(\tR\x04name\x12\x1e\n" +
	"\x06secret\x18\x03 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x06secret\"9\n" +
	"\x19OpenStackTokenCredentials\x12\x1c\n" +
	"\x05token\x18\x01 \x01(\tB\x06\xbaH\x03\xc8\x01\x01R\x05tokenB\xd0\x01\n" +
	"\x19com.dev.planton.openstackB\rProviderProtoP\x01Z.github.com/plantonhq/planton/catalog/openstack\xa2\x02\x03DPO\xaa\x02\x15Dev.Planton.Openstack\xca\x02\x15Dev\\Planton\\Openstack\xe2\x02!Dev\\Planton\\Openstack\\GPBMetadata\xea\x02\x17Dev::Planton::Openstackb\x06proto3"

var (
	file_catalog_openstack_provider_proto_rawDescOnce sync.Once
	file_catalog_openstack_provider_proto_rawDescData []byte
)

func file_catalog_openstack_provider_proto_rawDescGZIP() []byte {
	file_catalog_openstack_provider_proto_rawDescOnce.Do(func() {
		file_catalog_openstack_provider_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_catalog_openstack_provider_proto_rawDesc), len(file_catalog_openstack_provider_proto_rawDesc)))
	})
	return file_catalog_openstack_provider_proto_rawDescData
}

var file_catalog_openstack_provider_proto_msgTypes = make([]protoimpl.MessageInfo, 4)
var file_catalog_openstack_provider_proto_goTypes = []any{
	(*OpenStackProviderConfig)(nil),         // 0: dev.planton.openstack.OpenStackProviderConfig
	(*OpenStackPasswordCredentials)(nil),    // 1: dev.planton.openstack.OpenStackPasswordCredentials
	(*OpenStackApplicationCredentials)(nil), // 2: dev.planton.openstack.OpenStackApplicationCredentials
	(*OpenStackTokenCredentials)(nil),       // 3: dev.planton.openstack.OpenStackTokenCredentials
}
var file_catalog_openstack_provider_proto_depIdxs = []int32{
	1, // 0: dev.planton.openstack.OpenStackProviderConfig.password:type_name -> dev.planton.openstack.OpenStackPasswordCredentials
	2, // 1: dev.planton.openstack.OpenStackProviderConfig.application_credential:type_name -> dev.planton.openstack.OpenStackApplicationCredentials
	3, // 2: dev.planton.openstack.OpenStackProviderConfig.token:type_name -> dev.planton.openstack.OpenStackTokenCredentials
	3, // [3:3] is the sub-list for method output_type
	3, // [3:3] is the sub-list for method input_type
	3, // [3:3] is the sub-list for extension type_name
	3, // [3:3] is the sub-list for extension extendee
	0, // [0:3] is the sub-list for field type_name
}

func init() { file_catalog_openstack_provider_proto_init() }
func file_catalog_openstack_provider_proto_init() {
	if File_catalog_openstack_provider_proto != nil {
		return
	}
	file_catalog_openstack_provider_proto_msgTypes[0].OneofWrappers = []any{
		(*OpenStackProviderConfig_Password)(nil),
		(*OpenStackProviderConfig_ApplicationCredential)(nil),
		(*OpenStackProviderConfig_Token)(nil),
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_catalog_openstack_provider_proto_rawDesc), len(file_catalog_openstack_provider_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   4,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_catalog_openstack_provider_proto_goTypes,
		DependencyIndexes: file_catalog_openstack_provider_proto_depIdxs,
		MessageInfos:      file_catalog_openstack_provider_proto_msgTypes,
	}.Build()
	File_catalog_openstack_provider_proto = out.File
	file_catalog_openstack_provider_proto_goTypes = nil
	file_catalog_openstack_provider_proto_depIdxs = nil
}
