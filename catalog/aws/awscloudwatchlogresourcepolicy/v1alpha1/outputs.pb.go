// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.6
// 	protoc        (unknown)
// source: catalog/aws/awscloudwatchlogresourcepolicy/v1alpha1/outputs.proto

package awscloudwatchlogresourcepolicyv1alpha1

import (
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	sync "sync"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// AwsCloudwatchLogResourcePolicyStackOutputs captures the observable
// state of the resource policy after apply.
type AwsCloudwatchLogResourcePolicyStackOutputs struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// The policy's identity: the policy name (account scope) or the
	// target resource ARN (resource scope) - also the provider's import
	// ID (an ARN-shaped import ID selects the resource scope).
	PolicyId string `protobuf:"bytes,1,opt,name=policy_id,json=policyId,proto3" json:"policy_id,omitempty"`
	// The scope AWS recorded: ACCOUNT or RESOURCE.
	PolicyScope string `protobuf:"bytes,2,opt,name=policy_scope,json=policyScope,proto3" json:"policy_scope,omitempty"`
	// AWS's revision ID after the last apply - the optimistic-concurrency
	// token guarding the next update.
	RevisionId    string `protobuf:"bytes,3,opt,name=revision_id,json=revisionId,proto3" json:"revision_id,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *AwsCloudwatchLogResourcePolicyStackOutputs) Reset() {
	*x = AwsCloudwatchLogResourcePolicyStackOutputs{}
	mi := &file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AwsCloudwatchLogResourcePolicyStackOutputs) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AwsCloudwatchLogResourcePolicyStackOutputs) ProtoMessage() {}

func (x *AwsCloudwatchLogResourcePolicyStackOutputs) ProtoReflect() protoreflect.Message {
	mi := &file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

// Deprecated: Use AwsCloudwatchLogResourcePolicyStackOutputs.ProtoReflect.Descriptor instead.
func (*AwsCloudwatchLogResourcePolicyStackOutputs) Descriptor() ([]byte, []int) {
	return file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescGZIP(), []int{0}
}

func (x *AwsCloudwatchLogResourcePolicyStackOutputs) GetPolicyId() string {
	if x != nil {
		return x.PolicyId
	}
	return ""
}

func (x *AwsCloudwatchLogResourcePolicyStackOutputs) GetPolicyScope() string {
	if x != nil {
		return x.PolicyScope
	}
	return ""
}

func (x *AwsCloudwatchLogResourcePolicyStackOutputs) GetRevisionId() string {
	if x != nil {
		return x.RevisionId
	}
	return ""
}

var File_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto protoreflect.FileDescriptor

const file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDesc = "" +
	"\n" +
	"Acatalog/aws/awscloudwatchlogresourcepolicy/v1alpha1/outputs.proto\x127dev.planton.aws.awscloudwatchlogresourcepolicy.v1alpha1\"\x8d\x01\n" +
	"*AwsCloudwatchLogResourcePolicyStackOutputs\x12\x1b\n" +
	"\tpolicy_id\x18\x01 \x01(\tR\bpolicyId\x12!\n" +
	"\fpolicy_scope\x18\x02 \x01(\tR\vpolicyScope\x12\x1f\n" +
	"\vrevision_id\x18\x03 \x01(\tR\n" +
	"revisionIdB\xc5\x03\n" +
	";com.dev.planton.aws.awscloudwatchlogresourcepolicy.v1alpha1B\fOutputsProtoP\x01Zwgithub.com/plantonhq/planton/catalog/aws/awscloudwatchlogresourcepolicy/v1alpha1;awscloudwatchlogresourcepolicyv1alpha1\xa2\x02\x04DPAA\xaa\x027Dev.Planton.Aws.Awscloudwatchlogresourcepolicy.V1alpha1\xca\x027Dev\\Planton\\Aws\\Awscloudwatchlogresourcepolicy\\V1alpha1\xe2\x02CDev\\Planton\\Aws\\Awscloudwatchlogresourcepolicy\\V1alpha1\\GPBMetadata\xea\x02;Dev::Planton::Aws::Awscloudwatchlogresourcepolicy::V1alpha1b\x06proto3"

var (
	file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescOnce sync.Once
	file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescData []byte
)

func file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescGZIP() []byte {
	file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescOnce.Do(func() {
		file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescData = protoimpl.X.CompressGZIP(unsafe.Slice(unsafe.StringData(file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDesc), len(file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDesc)))
	})
	return file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDescData
}

var file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_goTypes = []any{
	(*AwsCloudwatchLogResourcePolicyStackOutputs)(nil), // 0: dev.planton.aws.awscloudwatchlogresourcepolicy.v1alpha1.AwsCloudwatchLogResourcePolicyStackOutputs
}
var file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_depIdxs = []int32{
	0, // [0:0] is the sub-list for method output_type
	0, // [0:0] is the sub-list for method input_type
	0, // [0:0] is the sub-list for extension type_name
	0, // [0:0] is the sub-list for extension extendee
	0, // [0:0] is the sub-list for field type_name
}

func init() { file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_init() }
func file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_init() {
	if File_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDesc), len(file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_goTypes,
		DependencyIndexes: file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_depIdxs,
		MessageInfos:      file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_msgTypes,
	}.Build()
	File_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto = out.File
	file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_goTypes = nil
	file_catalog_aws_awscloudwatchlogresourcepolicy_v1alpha1_outputs_proto_depIdxs = nil
}
