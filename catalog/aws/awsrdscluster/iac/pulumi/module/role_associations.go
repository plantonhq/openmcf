...
// specific engine capability (s3Import, Lambda, SageMaker, ...). AWS
// validates at AddRoleToDBCluster that the role's trust policy allows
// rds.amazonaws.com to assume it and 400s otherwise -- a server-side
// contract nothing at preview time catches. The cluster resource's
// inline IamRoles argument is deliberately unused: it cannot carry
// feature names and, per the provider's own warning, mixing it with
// association resources overwrites the associations.
