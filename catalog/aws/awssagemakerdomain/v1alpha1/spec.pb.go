...
const file_catalog_aws_awssagemakerdomain_v1alpha1_spec_proto_rawDesc = "" +
	"\n" +
	"2catalog/aws/awssagemakerdomain/v1alpha1/spec.proto\x12+dev.planton.aws.awssagemakerdomain.v1alpha1\x1a\x1bbuf/validate/validate.proto\x1a&shared/foreignkey/v1/foreign_key.proto\x1a\x1cshared/options/options.proto\"\x8f,\n" +
	...
	"\x06spaces\x18\x12 \x03(\v2D.dev.planton.aws.awssagemakerdomain.v1alpha1.AwsSagemakerDomainSpaceR\x06spaces:\xa0\x1c\xbaH\x9c\x1c\x1aU\n" +
	...
	"\x12space_names_unique\x128spaces names must be unique — each name keys one space\x1a)this.spaces.map(s, s.space_name).unique()\x1a\xf8\x05\n" +
	".space_idle_requires_owner_lifecycle_jupyterlab\x12\xbb\x01a space may not set jupyter_lab_app_settings.idle_settings when its owner profile sets lifecycle_management 'DISABLED' for JupyterLab — AWS rejects SpaceIdleSettings on a disabled plane\x1a\x87\x04this.spaces.all(s, !has(s.space_settings) || !has(s.space_settings.jupyter_lab_app_settings) || !has(s.space_settings.jupyter_lab_app_settings.idle_settings) || !has(s.ownership_settings) || !this.user_profiles.exists(p, p.user_profile_name == s.ownership_settings.owner_user_profile_name && has(p.user_settings) && has(p.user_settings.jupyter_lab_app_settings) && has(p.user_settings.jupyter_lab_app_settings.idle_settings) && p.user_settings.jupyter_lab_app_settings.idle_settings.lifecycle_management == 'DISABLED'))\x1a\xf9\x05\n" +
	".space_idle_requires_owner_lifecycle_codeeditor\x12\xbc\x01a space may not set code_editor_app_settings.idle_settings when its owner profile sets lifecycle_management 'DISABLED' for Code Editor — AWS rejects SpaceIdleSettings on a disabled plane\x1a\x87\x04this.spaces.all(s, !has(s.space_settings) || !has(s.space_settings.code_editor_app_settings) || !has(s.space_settings.code_editor_app_settings.idle_settings) || !has(s.ownership_settings) || !this.user_profiles.exists(p, p.user_profile_name == s.ownership_settings.owner_user_profile_name && has(p.user_settings) && has(p.user_settings.code_editor_app_settings) && has(p.user_settings.code_editor_app_settings.idle_settings) && p.user_settings.code_editor_app_settings.idle_settings.lifecycle_management == 'DISABLED'))B\x1a\n" +
