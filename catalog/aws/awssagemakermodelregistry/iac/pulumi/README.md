# AwsSagemakerModelRegistry — Pulumi module (Go)

Deploys an Amazon SageMaker model package group
(`sagemaker.ModelPackageGroup`) with its folded resource policy
...
- **The policy is an idempotent upsert** (`PutModelPackageGroupPolicy`)
  ...
  carries `resource_policy` (JSON-marshaled from the structured spec
  field), and removing it deletes the policy.
