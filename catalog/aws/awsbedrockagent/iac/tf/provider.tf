terraform {
  ...
  required_providers {
    aws = {
      ...
      #
      # Feature floor: the agent family resources predate the 6.x line;
      # within it, aws_bedrockagent_agent's idle_session_ttl_in_seconds
      # cap rises to 5400 in v6.46.0 (the spec validates that bound), so
      # an older floor would reject manifests the spec accepts.
