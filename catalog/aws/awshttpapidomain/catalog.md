...
**Routing rules route dynamically** -- switch `routingMode` to `ROUTING_RULE_THEN_API_MAPPING` (rules first, mappings as the fallback) or `ROUTING_RULE_ONLY`, then declare rules that match on base paths or header globs and invoke an API stage. Priorities (1-1,000,000, unique, lowest first) order evaluation; conditions on one rule are ANDed; `stripBasePath` forwards `/orders/list` to the API as `/list`. The spec rejects rules without a rule-honoring mode -- and a rule mode without rules -- at validate time.
