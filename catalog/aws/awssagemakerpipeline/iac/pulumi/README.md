# AwsSagemakerPipeline — Pulumi module (Go)

Deploys an Amazon SageMaker AI pipeline (`sagemaker.Pipeline`) — the ML
workflow DAG executions run against.
...
- **The definition comes from exactly one place** (spec-validated) —
  the inline arm is JSON-marshaled from the structured spec field; the
...
Outputs mirror the Terraform module key-for-key: `pipeline_name`,
`pipeline_arn`.
