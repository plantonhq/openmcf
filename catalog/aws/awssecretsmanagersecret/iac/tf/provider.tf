terraform {
  ...
  required_providers {
    aws = {
      ...
      #
      # Feature floor: the external rotation surface on
      # aws_secretsmanager_secret_rotation (external_secret_rotation_role_arn
      # and external_secret_rotation_metadata) lands in v6.55.0 -- an older
      # floor would silently reject the partner-managed rotation arm. (The
      # secret, version, and policy resources predate 6.x entirely.)
