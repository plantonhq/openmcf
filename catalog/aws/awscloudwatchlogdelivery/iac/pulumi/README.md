# AwsCloudwatchLogDelivery — Pulumi module (Go)

Manages the vended-log pipeline (`cloudwatch.LogDeliverySource`, `cloudwatch.LogDeliveryDestination` + policy, `cloudwatch.LogDelivery`) and/or the legacy cross-account destination (`cloudwatch.LogDestination` + policy).
...
- **Each delivery's destination is own-XOR-existing** (spec-guaranteed): owned destinations resolve to their resource's ARN with a DependsOn; external destinations pass the literal ARN through.
...
Outputs mirror the Terraform module key-for-key: `source_name`/`source_arn`/`source_service`, `destination_arns` (keyed by name), `delivery_ids`/`delivery_arns` (keyed by name), `cross_account_destination_name`/`_arn`.
