terraform {
  ...
  required_providers {
    aws = {
      ...
      #
      # Feature floor: aws_bedrockagent_flow is NEW in v6.26.0 -- any older
      # floor has no flow resource at all. The Loop node family's
      # configuration is not yet expressible at the pin (upstream gap).
