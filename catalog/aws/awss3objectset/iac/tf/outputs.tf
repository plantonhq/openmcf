...
output "object_arns" {
  ...
  value = merge(
    ...
    { for key, obj in aws_s3_object_copy.objects : key => obj.arn },
  )
}
...
output "object_etags" {
  ...
  value = merge(
    ...
    { for key, obj in aws_s3_object_copy.objects : key => obj.etag },
  )
}
...
output "object_version_ids" {
  ...
  value = merge(
    ...
    { for key, obj in aws_s3_object_copy.objects : key => obj.version_id },
  )
}
