...
- **`priority` orders queues, not jobs.** When queues share an environment, higher-priority queues win. Within one queue, ordering is FIFO — or fair-share when a [scheduling policy](../awsbatchschedulingpolicy/README.md) is attached.
