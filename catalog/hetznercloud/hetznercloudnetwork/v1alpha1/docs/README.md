...
Planton bundles the network, its subnets, and its routes into a single component because a Hetzner Cloud network is unusable without at least one subnet. Servers and load balancers attach to subnets, not to the network directly. Routes are tightly coupled to the network's address space and routing table. Splitting these into separate components would force every user to deploy at least two manifests (network + subnet) for the simplest private networking setup — unnecessary friction for a relationship that is always 1:N and never cross-referenced.
...
**Planton approach:** A single manifest that declares the network CIDR, subnets as a repeated list, and routes as an optional repeated list. The IaC modules handle resource creation order, ID passing between resources, and resource naming. The `network_id` output feeds into downstream components via `StringValueOrRef`, enabling declarative composition without hardcoded IDs.
...
**Verdict:** Production-grade for Terraform teams. The standard choice before Planton. The three-resource model is correct but verbose.
...
**A notable friction point:** The `Network` resource's `ID()` returns `IDOutput` (a string), but `NetworkSubnetArgs.NetworkId` and `NetworkRouteArgs.NetworkId` expect `IntInput`. This requires an `ApplyT` conversion at every reference site. The Planton Pulumi module handles this once in a shared variable.
...
**Verdict:** Excellent for Go/TypeScript teams and dynamic network generation. Planton uses Pulumi (Go) internally for its IaC modules.
...
| **Planton** | **Yes** | **Yes** | **Yes** | **Via VCS** | **Proto + CEL** |
...
The Hetzner Cloud network API has several attributes. Planton's `HetznerCloudNetworkSpec` exposes the attributes that matter for 80% of use cases.
...
**Subnet type as enum:** The `SubnetType` enum (`cloud`, `server`, `vswitch`) prevents invalid type strings. Terraform accepts any string for the `type` field and rejects invalid values at API call time. Planton rejects them at schema validation.
