...
**IaC management:** Terraform and Pulumi both support `hcloud_primary_ip` / `hcloud.PrimaryIp` as separate resources, with `hcloud_rdns` / `hcloud.Rdns` as a second resource for reverse DNS. This two-resource model is correct but introduces wiring: the rDNS resource needs the Primary IP's numeric ID and its allocated IP address, both of which are only known after creation. Planton's single-manifest approach handles this wiring internally.
...
**Verdict:** Production-grade for Terraform teams. The standard choice before Planton. The two-resource model for IP + rDNS is correct but introduces wiring that could be encapsulated.
...
**A notable friction point:** The `PrimaryIp` resource's `ID()` returns `IDOutput` (a string representation of the numeric ID), but `RdnsArgs.PrimaryIpId` expects `IntInput`. Every Pulumi user who wants rDNS on a Primary IP must write the `ApplyT` conversion. The Planton Pulumi module handles this once in the resource file.
...
| **Planton** | **Yes** | **Yes** | **Yes (single manifest)** | **Via VCS** | **Hardcoded safe defaults** |

The key differentiators for Planton:

1. **Single manifest**: One YAML declares the IP and its rDNS. No resource wiring.
2. **Safe defaults hardcoded**: `auto_delete=false` and `assignee_type="server"` are not exposed because there is only one correct answer in Planton's component model.
...
3. **Safe lifecycle defaults:** `auto_delete` is always `false`. In Planton, resources are managed independently — auto-deletion would silently destroy a separately-managed resource when a server is removed.
...
The Hetzner Cloud Primary IP API has several attributes. Planton's `HetznerCloudPrimaryIpSpec` exposes the attributes that matter for 80% of use cases.
...
- `auto_delete = false` — Resources in Planton are independently managed. Auto-deletion on server removal would silently destroy a separately-managed resource, violating the component model's guarantee that deleting component A does not affect component B.
...
- `datacenter` — Deprecated in the provider (removal after 2026-07-01). The `location` field supersedes it. Planton does not expose deprecated fields.
...
**IpType as a proto enum:** The `type` field uses an enum (`ipv4`, `ipv6`) rather than a string. This prevents invalid type strings at schema validation time. Terraform accepts any string and rejects invalid values at API call time. Planton rejects them at proto validation.
