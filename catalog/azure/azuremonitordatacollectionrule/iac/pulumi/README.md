# AzureMonitorDataCollectionRule Pulumi Module
...
- `monitoring.DataCollectionRule` -- the rule with its data sources, destinations, data flows, stream declarations, and identity
...
- **The Event Hub data import is a single block by design** -- Azure's rule model carries exactly one; the classic SDK's array receives exactly one element (the bridged provider would silently drop extras).
- **ENGINE-SHAPE**: the classic SDK types `azure_monitor_metrics` / `event_hub` / `event_hub_direct` as singular pointers -- the same one-entry cap the Terraform provider enforces with MaxItems, so both engines carry the identical shape.
