...
type Locals struct {
	AzureMachineLearningBatchDeployment *azuremachinelearningbatchdeploymentv1alpha1.AzureMachineLearningBatchDeployment
...
// parseEndpointId splits a batch endpoint ARM ID into its
// resource-group, workspace and endpoint names. The ID shape is fixed
// by ARM:
// /subscriptions/{sub}/resourceGroups/{rg}/providers/Microsoft.MachineLearningServices/workspaces/{ws}/batchEndpoints/{endpoint}
func parseEndpointId(endpointId string) (resourceGroupName, workspaceName, endpointName string, err error) {
	...
	if resourceGroupName == "" || workspaceName == "" || endpointName == "" {
		return "", "", "", fmt.Errorf("endpoint id %q does not carry resourceGroups, workspaces and batchEndpoints segments", endpointId)
	}
...
func initializeLocals(ctx *pulumi.Context, stackInput *azuremachinelearningbatchdeploymentv1alpha1.AzureMachineLearningBatchDeploymentStackInput) *Locals {
	...
	locals.AzureTags = map[string]string{
		...
		"resource_kind": strings.ToLower(cloudresourcekind.CloudResourceKind_AzureMachineLearningBatchDeployment.String()),
	}
