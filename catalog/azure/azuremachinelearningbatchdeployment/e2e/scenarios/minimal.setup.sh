...
#
# The batch service refuses to create a Model-type recipe without a model
# reference (live-proven boundary; the verbatim validator errors are
# recorded on the spec's model and compute_id fields). A registered model
# is data-plane content -- files uploaded into the fixture workspace --
# which no catalog kind can create, so this script registers one here.
#
...
# the lane's inputs can never drift. The same seed serves the online
# deployment's lane; each kind carries its own copy of this script because
# a scenario referencing a sibling kind's files would couple the two
# folders invisibly.
#
# Idempotent: skips registration when the fixture workspace already holds
# the model version. Everything seeded here lives inside the fixture
# workspace and dies with it at DEPENDENCIES-DOWN.
set -euo pipefail
