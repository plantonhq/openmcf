locals {
  ...
  base_tags = {
    ...
    "resource_kind" = "azure_traffic_manager_profile"
  ...
  # The provider's two-value Enabled/Disabled vocabulary, driven by the
  # spec's bool (unset applies true, the provider's own default).
  profile_status = coalesce(var.spec.enabled, true) ? "Enabled" : "Disabled"
}
