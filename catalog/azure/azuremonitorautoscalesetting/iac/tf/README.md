# AzureMonitorAutoscaleSetting Terraform Module
...
- `azurerm_monitor_autoscale_setting` -- the setting with its profiles, rules, schedules, predictive configuration, and notifications
...
- `spec` -- the AzureMonitorAutoscaleSettingSpec fields; the resource group, target, and metric-source references arrive as resolved literals
...
- **`enabled` is always sent explicitly** (platform default true) so plans stay deterministic.
- **Omitting `predictive` IS the disabled state** -- the API exposes no Disabled mode (provider contract, mirrored from its read path). Predictive applies to VM Scale Set targets only.
...
- **Recurrence `hour`/`minute` are single values in the spec** -- the provider takes one-item lists; the module wraps them.
