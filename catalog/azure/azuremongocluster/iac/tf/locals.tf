locals {
  ...
  base_tags = {
    ...
    "resource_kind" = "azure_mongo_cluster"
  ...
  # Composed firewall rules keyed by rule name -- renames replace only
  # that rule; sibling rules stay untouched (lockstep with the Pulumi
  # module's per-name resources).
  firewall_rules_by_name = { for rule in var.spec.firewall_rules : rule.name => rule }
}
