...
func initializeLocals(ctx *pulumi.Context, stackInput *azurecomputegalleryimagev1alpha1.AzureComputeGalleryImageStackInput) *Locals {
	...
	locals.MetadataTags = map[string]string{
		...
		"resource_kind": strings.ToLower(cloudresourcekind.CloudResourceKind_AzureComputeGalleryImage.String()),
	}

	if target.Metadata.Id != "" {
		locals.MetadataTags["resource_id"] = target.Metadata.Id
	}

	if target.Metadata.Org != "" {
		locals.MetadataTags["organization"] = target.Metadata.Org
	}

	if target.Metadata.Env != "" {
		locals.MetadataTags["environment"] = target.Metadata.Env
	}
