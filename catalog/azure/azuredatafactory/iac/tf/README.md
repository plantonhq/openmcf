# AzureDataFactory Terraform Module
...
- `azurerm_data_factory` -- the factory (identity, repo binding, global parameters, network posture, inline customer-managed-key encryption)
- `azurerm_data_factory_credential_user_managed_identity` -- one per named entry in `user_managed_identity_credentials`, keyed by name
- `azurerm_data_factory_credential_service_principal` -- one per named entry in `service_principal_credentials`, keyed by name
- `azurerm_data_factory_managed_private_endpoint` -- one per named entry in `managed_private_endpoints`, keyed by name
...
- `spec` -- the AzureDataFactorySpec fields; the resource group, identity, key, and target references arrive as resolved literals
...
- **The managed virtual network is one-way**: enabling it updates in place (the provider creates the managed network named "default"); disabling it REPLACES the factory (the provider's one CustomizeDiff).
