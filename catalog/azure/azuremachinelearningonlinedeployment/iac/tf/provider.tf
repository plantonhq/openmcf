...
# azurerm carries NO resource for Machine Learning online deployments
# (its endpoint draft is tracked at
# hashicorp/terraform-provider-azurerm#32011). The resource is written at
# the pinned ARM api-version 2025-06-01 -- the same GA line the ML
# family's verification pins -- and the kind's spec carries the full
# validation burden (azapi has no provider-side schema).
#
