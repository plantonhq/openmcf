...
func TestAzureApplicationSecurityGroupSpec(t *testing.T) {
	gomega.RegisterFailHandler(ginkgo.Fail)
	ginkgo.RunSpecs(t, "AzureApplicationSecurityGroupSpec Validation Tests")
}
...
// validResource returns a minimal valid AzureApplicationSecurityGroup that
// individual cases then mutate into the shape under test.
func validResource() *AzureApplicationSecurityGroup {
...
var _ = ginkgo.Describe("AzureApplicationSecurityGroupSpec Validation Tests", func() {

	ginkgo.Describe("When valid input is passed", func() {
		ginkgo.Context("azure_application_security_group", func() {
			...
			ginkgo.It("should accept a name with periods, hyphens, and underscores", func() {
				input := validResource()
				input.Spec.Name = "app.tier-1_grp"
				err := protovalidate.Validate(input)
				gomega.Expect(err).To(gomega.BeNil())
			})
