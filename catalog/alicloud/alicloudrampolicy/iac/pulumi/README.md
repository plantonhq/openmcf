...
Deploys an Alibaba Cloud RAM custom policy with a JSON permission document, optional version rotation, and tag management. The component provisions a single `alicloud_ram_policy` resource that can be referenced by AliCloudRamRole for attachment to roles.
...
| `policy_type` | `string` | Always `Custom` for user-created policies. Used by AliCloudRamRole `policyAttachments` which require both name and type. |
