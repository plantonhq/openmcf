...
- [OciSecurityGroup](/docs/catalog/oci/ocisecuritygroup) — manages network security rules for the NLB via `networkSecurityGroupIds`
- [OciApplicationLoadBalancer](/docs/catalog/oci/ociapplicationloadbalancer) — the Layer 7 counterpart for HTTP/HTTPS workloads with SSL termination, hostname routing, and rule sets
- [OciPublicIp](/docs/catalog/oci/ocipublicip) — provides reserved public IPs that can be assigned to the NLB via `reservedIps`
