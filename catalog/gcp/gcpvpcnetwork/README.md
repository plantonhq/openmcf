...
This component is part of Planton's infrastructure-as-code framework, offering a simple protobuf-based API that generates production-ready Terraform and Pulumi code for GCP VPC deployment.
...
- **Multi-Cloud Consistency**: Part of Planton's unified infrastructure API across GCP, AWS, and Azure
- **Infrastructure-as-Code Native**: Generates Terraform or Pulumi code with proper state management and drift detection
- **Foreign Key Support**: Reference other Planton resources (like `GcpProject`) directly for clean dependency management
...
The `GcpVpcNetwork` resource follows Planton's standard Kubernetes-style API structure:
...
| `routing_mode` | GcpVpcNetworkRoutingMode | No | `REGIONAL` | Dynamic routing mode for Cloud Routers. Use `GLOBAL` for multi-region routing |
...
For detailed rationale behind design decisions, network provisioning evolution, and comparison of IaC tools, see the research documentation.
...
- **VPC Peering**: Set up peering via `GcpVpcNetworkPeering` resources
...
- [`GcpSubnetwork`](../gcpsubnetwork/README.md) - Create subnets within this VPC
- [`GcpGkeCluster`](../gcpgkecluster/README.md) - Deploy GKE clusters using this VPC
- [`GcpProject`](../gcpproject/README.md) - Manage GCP projects
