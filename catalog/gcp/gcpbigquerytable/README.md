...
| `deletionPolicy` | `string` | No | `DELETE` (default; still gated by `deletionProtection`), `PREVENT` (destroy fails outright), `ABANDON` (drops the table from management, bypassing the guard). |
| `ignoreAutoGeneratedSchema` | `bool` | No | Hide diffs from columns BigQuery adds on its own. |
| `ignoreSchemaChanges` | `string[]` | No | Schema sub-fields treated as non-authoritative (`dataPolicies`). |
| `tableMetadataView` | `string` | No | Read-tuning: `BASIC`, `STORAGE_STATS` (API default), or `FULL`. |
