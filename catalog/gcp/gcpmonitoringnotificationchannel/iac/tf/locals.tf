locals {
  ...
  # created it. These land on user_labels (freeform metadata) — NEVER on the
  # provider's `labels` argument, which is the type-specific channel
  # configuration fed from spec.channel_labels.
  base_labels = {
    "planton-ai_resource" = "true"
    "planton-ai_name"     = var.metadata.name
    "planton-ai_kind"     = "gcpmonitoringnotificationchannel"
  }
