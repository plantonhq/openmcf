locals {
  ...
  # The composed IdP configs keyed for for_each: default-supported by the
  # provider's canonical IdP ID, OIDC and SAML by their user-chosen resource
  # names — all immutable identity keys, so plans stay stable as list order
  # changes.
