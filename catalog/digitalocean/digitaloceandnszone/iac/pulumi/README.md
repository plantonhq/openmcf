...
This directory contains the Pulumi (Go) implementation for provisioning DigitalOcean DNS Zones through the Planton API. The implementation reads a `DigitalOceanDnsZone` protobuf manifest and deploys the corresponding DNS zone and records using the DigitalOcean Pulumi provider.
...
For production, use the Planton CLI instead of running Pulumi directly:
