...
- **permission scopes**: Each permission references a scope by `name` and the `resource_server_identifier` (audience) of the API that owns it. The scope must already exist on that resource server (see [Auth0ResourceServer](../auth0resourceserver/README.md)).
