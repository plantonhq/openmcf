...
This violates OpenMCF's "80/20 principle"—these are essential configuration options that a significant percentage of production users need.
...
message KubernetesTemporalDynamicConfig {
  ...
  optional int64 history_size_limit_error = 1 [(org.openmcf.shared.options.default) = "52428800"];
  
  // Maximum number of events in workflow execution history
  // Default: 51200. Increase for workflows with many activities/signals.
  optional int64 history_count_limit_error = 2 [(org.openmcf.shared.options.default) = "51200"];
...
message KubernetesTemporalSpec {
  ...
  optional int32 num_history_shards = 13 [(org.openmcf.shared.options.default) = "512"];
}
```
...
message KubernetesTemporalServiceConfig {
  // Number of replicas
  optional int32 replicas = 1 [(org.openmcf.shared.options.default) = "1"];
...
Users must:
- Manually patch Helm releases after OpenMCF deployment
...
- Follow OpenMCF's standard validation-first workflow
...
- OpenMCF [80/20 design philosophy](https://github.com/plantonhq/openmcf/blob/main/architecture/README.md#the-8020-principle)

## Priority

**Medium-High** — This blocks production use cases where workflows exceed default limits. The workarounds (manual Helm patching, module forking) defeat the purpose of using OpenMCF.
