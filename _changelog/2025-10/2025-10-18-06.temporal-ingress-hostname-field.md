...
message TemporalKubernetesSpec {
  org.openmcf.shared.kubernetes.IngressSpec ingress = 6;
}
```
...
2. **Updated Field Type**: Line 42 changed from `org.openmcf.shared.kubernetes.IngressSpec` to `TemporalKubernetesIngress`
...
```bash
# Preview changes
openmcf pulumi preview --manifest temporal.yaml
...
# Deploy
openmcf pulumi up --manifest temporal-test.yaml
...
# Deploy
openmcf pulumi up --manifest temporal-frontend-only.yaml
...
# Attempt deploy
openmcf pulumi up --manifest temporal-invalid.yaml
