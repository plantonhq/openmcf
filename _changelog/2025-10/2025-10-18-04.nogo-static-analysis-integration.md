...
[2,628 / 2,635] Running nogo on //cmd/openmcf:openmcf
...
- Calls `go_sdk.nogo(nogo = "//:openmcf_nogo")` to register analyzer
...
```bash
cd ~/scm/github.com/plantonhq/openmcf && go vet ./...
# Exit code: 0
# ✅ PASS - No errors
```
...
```bash
./bazelw build --config=bb --remote_header=x-buildbuddy-api-key=$OPENMCF_BUILDBUDDY_API_KEY //:openmcf
```

**Output**:
```
INFO: Analyzed target //:openmcf (0 packages loaded, 0 targets configured).
...
```bash
~/scm/github.com/plantonhq/openmcf  make build
...
```
[2,628 / 2,635] Running nogo on //cmd/openmcf:openmcf; 1s darwin-sandbox
INFO: 524 total actions
```

**Subsequent Builds** (warm cache):
```
[2,628 / 2,635] Running nogo on //cmd/openmcf:openmcf; 0s remote-cache
...
**3. MODULE.bazel**:
- Added `go_sdk.nogo(nogo = "//:openmcf_nogo")` call
...
bazel build --define=nogo=off //:openmcf

# Enable only critical analyzers
bazel build --define=nogo=critical //:openmcf
```
