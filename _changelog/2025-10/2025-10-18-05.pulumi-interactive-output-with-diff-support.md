...
When running Pulumi commands through `openmcf`, users were seeing plain streaming output instead of Pulumi's beautiful interactive tree view that shows resources in a hierarchical structure:
...
@ Previewing update.....
    pulumi:pulumi:Stack openmcf-kubernetes.PostgresKubernetes.test-swarup  Compiling the program ...
@ Previewing update....
    pulumi:pulumi:Stack openmcf-kubernetes.PostgresKubernetes.test-swarup  Finished compiling
 +  pulumi:pulumi:Stack openmcf-kubernetes.PostgresKubernetes.test-swarup create Finished compiling
@ Previewing update....
...
- Frustration when comparing openmcf output to native Pulumi experience
...
**Command**:
```bash
openmcf pulumi preview --manifest postgres.yaml --module-dir ${MODULE}
```
...
**Command**:
```bash
openmcf pulumi preview --manifest postgres.yaml --module-dir ${MODULE} --diff
```
...
**Command**:
```bash
openmcf pulumi up --manifest postgres.yaml --module-dir ${MODULE} --yes
```
...
**File**: `cmd/openmcf/root/pulumi.go`
...
**Files**:
- `cmd/openmcf/root/pulumi/preview.go`
- `cmd/openmcf/root/pulumi/update.go`
- `cmd/openmcf/root/pulumi/destroy.go`
- `cmd/openmcf/root/pulumi/refresh.go`
...
**Before**: openmcf forced non-interactive mode, diverging from standard Pulumi UX

**After**: openmcf respects Pulumi's output modes, providing the same experience as native Pulumi CLI
...
```
User runs openmcf pulumi command
    ↓
Is --yes flag present?
    ├─ Yes → Add --yes flag
...
- Interactive tree view rendering happens in Pulumi CLI (no overhead for openmcf)
...
openmcf pulumi preview --manifest postgres-test-swarup.yaml --module-dir ${POSTGRES_MODULE}
```
...
```bash
openmcf pulumi preview --manifest postgres-test-swarup.yaml --module-dir ${POSTGRES_MODULE} --diff
```
...
```bash
openmcf pulumi up --manifest postgres-test-swarup.yaml --module-dir ${POSTGRES_MODULE} --yes
```
...
**Result**: ✅ Binary rebuilt and installed to `~/bin/openmcf`
...
2. **`cmd/openmcf/root/pulumi.go`**: Registered `--diff` flag (1 line)
...
4. **`cmd/openmcf/root/pulumi/preview.go`**: Read and pass `--diff` flag (2 lines added)
5. **`cmd/openmcf/root/pulumi/update.go`**: Read and pass `--diff` flag (2 lines added)
6. **`cmd/openmcf/root/pulumi/destroy.go`**: Read and pass `--diff` flag (2 lines added)
7. **`cmd/openmcf/root/pulumi/refresh.go`**: Read and pass `--diff` flag (2 lines added)
...
openmcf pulumi preview --manifest file.yaml

# To see what changed: Add --diff
openmcf pulumi preview --manifest file.yaml --diff

# For rapid iteration: Auto-approve with visibility
openmcf pulumi up --manifest file.yaml --yes

# For maximum visibility: Combine flags
openmcf pulumi up --manifest file.yaml --yes --diff
```
...
which openmcf
# Should be: ~/bin/openmcf

# 4. Don't redirect output
openmcf pulumi preview ... > output.txt  # ❌ Breaks TTY
openmcf pulumi preview ...                # ✅ Works
```
...
```bash
openmcf pulumi preview --output-format tree    # Interactive tree (default)
openmcf pulumi preview --output-format stream  # Streaming (force)
openmcf pulumi preview --output-format json    # JSON output
```
...
metadata:
  labels:
    pulumi.openmcf.org/output-format: tree
    pulumi.openmcf.org/show-diff: "true"
```
...
```bash
openmcf pulumi preview --manifest file.yaml --save-output preview.log
