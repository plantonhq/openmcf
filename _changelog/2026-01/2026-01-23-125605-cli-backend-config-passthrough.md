...
When running unified commands like `project-planton preview` with CLI backend flags:

```bash
project-planton preview -i manifest.yaml --backend-type=s3 --local-module
```
...
5. The manifest had `terraform.project-planton.org/backend.key` but NO `backend.type`
...
| `cmd/project-planton/root/tofu/apply.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/tofu/destroy.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/tofu/plan.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/tofu/refresh.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/terraform/apply.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/terraform/destroy.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/terraform/plan.go` | Pass `nil` for backendConfig |
| `cmd/project-planton/root/terraform/refresh.go` | Pass `nil` for backendConfig |

## Expected Behavior After Fix

1. User runs: `project-planton preview -i manifest.yaml --backend-type=s3 --local-module`
