...
Added Java protobuf stubs generation to the project-planton build pipeline to catch Java reserved keyword violations early, before they propagate to downstream consumers. This prevents broken builds in the planton monorepo by detecting issues like using `static` or `class` as enum values during local development.
...
Added Java stubs generation to the project-planton build pipeline as an early warning system.
...
**`apis/_rules/avoid-reserved-words-in-project-planton-apis.mdc`** - New cursor rule:
