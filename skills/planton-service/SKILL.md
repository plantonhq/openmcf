...
| `references/preview-environments.md` | Per-pull-request preview environments: the pullRequests.deploy opt-in, the `{service}-pr-{number}` environment record and its base-environment pointer, reading a preview run's deploy states honestly (including that the preview deploy lane is not yet open), the concurrency cap and expiry |
