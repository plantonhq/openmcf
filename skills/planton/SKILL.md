...
2. Map each resource to its cloud resource kind. The multi-cloud-catalog
   ...
   the RIGHT kind even when the organization's catalog policy disables it
   -- availability never truncates a design; the disclosure happens before
   deploy, not here (`references/catalog.availability.md`).
...
- **One configuration home, two writers.** On a git-maintained service
  (one declaring `deploy.kustomize`) the platform's own build lane writes
  `deploy.environments` -- user applies preserve the stored section rather
  than overwrite it. On a manually-declared service, the caller writes it.
  Nothing else ever does (`references/service.configuring-deployments.md`).
