...
On the platform-tools arm the twin is `validate_service_pipeline`: submit `files` (a path-to-content map of the pipeline file — by default `.planton/pipeline.yaml` — plus every Task file beside it under its `tasks/` directory) with the pipeline's own `params`, or `track` to validate a platform track. It runs the same compiler with the same stand-in rule and answers with the same JSON; verdicts arrive in the report (`valid: false`, `errors`), never as a tool error. Pass `org` and task references the files do not answer are resolved from the organization's published `TektonTask` records automatically — the twin needs no `--task` stand-ins. A missing `pipeline_path` entry, both or neither source, or an oversized payload is a malformed call, not a verdict: fix the call.
