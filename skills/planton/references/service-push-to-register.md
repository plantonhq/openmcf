...
1. **The file is the opt-in.** No setting enables this lane — the connected GitHub App installation is the trust anchor (connecting it was an org-admin act), and committing a Planton `service.yaml` is the declaration of intent. The manifest grammar IS the Service resource's YAML: the same document the console shows, the API accepts, and `planton service register` submits.
