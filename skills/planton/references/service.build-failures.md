...
- `building docker-auth secret: resolving gcp artifact registry service account key: ...` / `... resolving ghcr username: ...` / `... resolving ghcr personal access token: ...` — the registry connection's referenced secret cannot be read. Fix the secret reference or the secret; rerun.
- `building docker-auth secret: exchanging ecr authorization token: ...` / `... no authorization data returned from AWS ECR` — the ECR credential is invalid. Fix the credential; rerun. (ECR is the only registry validated at dispatch; a wrong-but-well-formed GCP or GHCR credential fails later, at the push step — class 5.)
