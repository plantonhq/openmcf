...
- Kubernetes 1.24+
- Helm 3.x
- The runner's identity document (minted by the control plane when the runner enrolls; the token-based install — the chart taking a runner token and the runner enrolling itself on arrival — ships with the enrollment rollout)
...
|-----------|-------------|---------|
| `credentials.content` | Raw JSON content of the runner's identity document | `""` |
...
The credentials file is the runner's identity document — the JSON the control
plane mints when the runner enrolls. It contains the runner's identity, mTLS
certificates, API key, and endpoint configuration in a single file. The chart
stores this as a Kubernetes Secret and mounts it into the pod at
`/etc/planton/credentials/credentials.json`.
