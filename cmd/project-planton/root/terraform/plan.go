...
func init() {
	Plan.PersistentFlags().Bool(string(flag.Destroy), false, "Create a destroy plan instead of apply plan")
	...
	Plan.PersistentFlags().Bool(string(flag.NoCleanup), false, "Do not cleanup the workspace copy after execution")
}
