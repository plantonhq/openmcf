'use client';

import { FC } from 'react';
import Link from 'next/link';
import { motion } from 'framer-motion';

interface ProgressStep {
  id: string;
  label: string;
  completed: boolean;
  active: boolean;
}

interface HeaderProps {
  /** Progress steps for the journey indicator */
  progress?: ProgressStep[];
}

/**
 * Minimal header for explainer pages.
 * 
 * Design philosophy:
 * - Just the "P" logo on the left, linking to planton.ai home
 * - Progress indicator in the center showing journey position
 * - No inter-page navigation - keeps focus on the current content
 * - Clean, unobtrusive presence
 */
export const Header: FC<HeaderProps> = ({ progress }) => {
  return (
    <motion.header
      initial={{ opacity: 0, y: -10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.4, ease: 'easeOut' }}
      className="sticky top-0 z-50 backdrop-blur-md bg-[#0a0a0a]/80 border-b border-white/5"
    >
      <div className="w-full max-w-5xl mx-auto px-4 sm:px-6">
        <div className="flex items-center justify-between h-12 sm:h-14">
          {/* Logo */}
          <Link
            href="/"
            className="text-white/80 hover:text-white transition-colors duration-200"
            aria-label="Planton Home"
          >
            <PlantonLogo />
          </Link>

          {/* Center: Progress indicator */}
          {progress && progress.length > 0 && (
            <ProgressIndicator steps={progress} />
          )}

          {/* Empty spacer for balance */}
          <div className="w-8" />
        </div>
      </div>
    </motion.header>
  );
};

// ============================================================================
// PLANTON LOGO
// ============================================================================

/**
 * The "P" logo SVG for Planton.
 */
const PlantonLogo: FC = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      width="28"
      height="28"
      viewBox="0 0 28 32"
      fill="none"
      className="text-current"
    >
      <path
        d="M11.8461 -0.154297H6.59239C-0.485 -0.154297 0.00721739 5.77323 0.00721739 5.77323V28.8139C0.00316622 28.8862 0.000465445 28.9542 0.000465445 29.025C0.000465445 29.0959 0.000465446 29.1696 0.00721739 29.2361C0.0587062 30.0458 0.401948 30.8025 0.964535 31.3464C1.52712 31.8904 2.26507 32.1791 3.0227 32.1517C3.78032 32.1244 4.4984 31.783 5.02544 31.1997C5.55248 30.6163 5.84728 29.8366 5.84765 29.025C5.84765 28.8197 5.79364 9.94707 5.79364 9.4201C5.79364 9.29577 5.79701 9.17577 5.80309 9.05867C5.80309 0.857719 11.8461 -0.154297 11.8461 -0.154297Z"
        fill="currentColor"
      />
      <path
        d="M18.2953 -0.154297H14.8031C14.8031 -0.154297 8.61536 0.844532 8.61536 8.93862C8.81793 5.35068 15.8222 5.69599 15.8222 5.69599H16.5309C17.8482 5.69618 19.1145 6.22194 20.0649 7.16331C21.0152 8.10468 21.5761 9.38876 21.6302 10.747C21.6843 12.1053 21.2275 13.4325 20.3552 14.4512C19.4829 15.4699 18.2627 16.1012 16.9499 16.2129C16.8116 16.2244 16.6733 16.2322 16.5309 16.2322H14.7333C14.7333 16.2322 10.4655 15.9361 10.4655 19.3878C10.4655 22.8395 15.3051 22.4478 15.3051 22.4478H18.2939C18.5013 22.4478 18.7087 22.4385 18.9127 22.425C23.9846 22.0953 28.0001 17.2553 28.0001 11.201C28.0001 5.14663 23.6555 -0.154297 18.2953 -0.154297Z"
        fill="currentColor"
      />
    </svg>
  );
};

// ============================================================================
// PROGRESS INDICATOR
// ============================================================================

interface ProgressIndicatorProps {
  steps: ProgressStep[];
}

/**
 * Minimal progress dots showing journey completion.
 * 
 * - Completed steps: filled pink dot
 * - Active step: pulsing pink dot with ring
 * - Pending steps: dim hollow dot
 */
const ProgressIndicator: FC<ProgressIndicatorProps> = ({ steps }) => {
  return (
    <div 
      className="flex items-center gap-1.5"
      role="progressbar"
      aria-label="Journey progress"
    >
      {steps.map((step, index) => (
        <motion.div
          key={step.id}
          initial={{ scale: 0.8, opacity: 0 }}
          animate={{ scale: 1, opacity: 1 }}
          transition={{ delay: index * 0.05, duration: 0.2 }}
          className="relative group"
        >
          {/* Tooltip on hover */}
          <div className="
            absolute bottom-full left-1/2 -translate-x-1/2 mb-2
            px-2 py-1 rounded-md bg-white/10 backdrop-blur-sm
            text-[10px] text-white/80 whitespace-nowrap
            opacity-0 group-hover:opacity-100 transition-opacity duration-200
            pointer-events-none
          ">
            {step.label}
          </div>
          
          {/* Dot */}
          <div
            className={`
              w-2 h-2 rounded-full transition-all duration-300
              ${step.completed
                ? 'bg-white'
                : step.active
                  ? 'bg-white/50 ring-2 ring-white/30 ring-offset-1 ring-offset-transparent'
                  : 'bg-white/20'
              }
            `}
          />
          
          {/* Active pulse animation */}
          {step.active && (
            <motion.div
              className="absolute inset-0 rounded-full bg-white/30"
              animate={{ scale: [1, 1.8, 1], opacity: [0.5, 0, 0.5] }}
              transition={{ duration: 2, repeat: Infinity, ease: 'easeInOut' }}
            />
          )}
        </motion.div>
      ))}
    </div>
  );
};

export default Header;
export type { ProgressStep };
