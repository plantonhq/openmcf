...
import { SELF_HOSTED_LICENSE_RUNGS, SELF_SERVE_SEAT_CEILING } from '@/data/pricing';
...
export const EnterpriseHero: FC = () => {
  const rungLow = SELF_HOSTED_LICENSE_RUNGS[0].usdPerYear;
  const rungHigh = SELF_HOSTED_LICENSE_RUNGS[SELF_HOSTED_LICENSE_RUNGS.length - 1].usdPerYear;
  return (
    <Stack className="items-center gap-5 pt-14 pb-4 px-4 bg-[#0a0a0a] text-center">
      ...
      <Typography className="text-sm md:text-base text-[#a0a0a0] max-w-[720px]">
        {`Under ${SELF_SERVE_SEAT_CEILING} seats, you don't need to talk to us at all — the self-serve license is $${(rungLow / 1000).toFixed(0)}K–$${(rungHigh / 1000).toFixed(0)}K a year, card and email, running today. Enterprise adds the things procurement actually needs: your identity provider, air-gap, compliance reporting, and a real SLA — at a published price.`}
      </Typography>
    </Stack>
  );
};
