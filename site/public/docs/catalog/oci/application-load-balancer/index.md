...
description: "Application Load Balancer deployment documentation"
icon: "package"
order: 100
componentName: "ociapplicationloadbalancer"
---
...
- [OciSubnet](/docs/catalog/oci/subnet) — provides subnets referenced by `subnetIds` via `valueFrom`
- [OciSecurityGroup](/docs/catalog/oci/network-security-group) — manages network security rules applied to the load balancer (`networkSecurityGroupIds`) via `valueFrom`
- [OciNetworkLoadBalancer](/docs/catalog/oci/network-load-balancer) — Layer 4 (TCP/UDP) load balancing with source IP preservation, for workloads that do not need HTTP-level features
