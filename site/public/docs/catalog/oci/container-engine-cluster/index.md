---
title: "Container Engine Cluster"
description: "Container Engine Cluster deployment documentation"
icon: "package"
order: 100
componentName: "ocicontainerenginecluster"
---
...
- [OciVcn](/docs/catalog/oci/vcn) — provides the VCN referenced by `vcnId` via `valueFrom`
- [OciSubnet](/docs/catalog/oci/subnet) — provides subnets for the API endpoint (`endpointConfig.subnetId`) and service load balancers (`options.serviceLbSubnetIds`) via `valueFrom`
- [OciSecurityGroup](/docs/catalog/oci/network-security-group) — manages network security rules for the API endpoint (`endpointConfig.nsgIds`) and service load balancer backends (`options.serviceLbConfig.backendNsgIds`) via `valueFrom`
- [OciContainerEngineNodePool](/docs/catalog/oci/container-engine-node-pool) — creates worker nodes for this cluster using the `cluster_id` output
