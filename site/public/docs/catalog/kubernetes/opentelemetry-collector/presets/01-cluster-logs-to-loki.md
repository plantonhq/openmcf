...
description: "The per-node log pipeline: daemonset mode puts one collector on every node, the filelog receiver tails every container's log files under `/var/log/pods` (the standard `container` operator parses the..."
type: "preset"
rank: "01"
presetSlug: "01-cluster-logs-to-loki"
