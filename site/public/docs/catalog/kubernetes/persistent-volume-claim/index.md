...
description: "Persistent Volume Claim deployment documentation"
icon: "package"
order: 100
componentName: "kubernetespersistentvolumeclaim"
---
...
- [KubernetesStorageClass](/docs/catalog/kubernetes/storage-class) — defines the storage tier; reference it from `spec.storage_class_name` so both deploy in one run
- [KubernetesNamespace](/docs/catalog/kubernetes/namespace) — provides the target namespace; reference it from `spec.namespace`
- [KubernetesDeployment](/docs/catalog/kubernetes/deployment) — mounts the claim by name in its volume mounts
- [KubernetesStatefulSet](/docs/catalog/kubernetes/statefulset) — for per-replica storage, use its `volume_claim_templates` instead of standalone claims
