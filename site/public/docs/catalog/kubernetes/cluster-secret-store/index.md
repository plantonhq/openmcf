...
Open the deployment store, find **Cluster Secret Store**, and click **Deploy**. The creation wizard walks you through the secrets home, the backend connection, authentication, tuning, and the namespace access fence. Start from a backend preset in the [Presets](#presets) tab.
...
This creates a cluster-wide connection to AWS Secrets Manager authenticating keylessly through the referenced ServiceAccount's IRSA binding.
...
**The secrets namespace** -- a cluster-scoped store reads its referenced credential Secrets from one explicit namespace (`secrets_namespace`); the operator's install namespace is the convention. Reference your External Secrets Operator resource to inherit its namespace output — the reference also draws the dependency edge.
...
|-------|-----------|---------|
| `spec.secrets_namespace` | KubernetesExternalSecretsOperator (`status.outputs.namespace`) | Where this store's credential Secrets are materialized |
...
|--------|-------------|----------------------|
| `store_name` | Name of the created ClusterSecretStore | Kubernetes External Secret's `store_ref.name` (with kind ClusterSecretStore) |
...
**AWS Secrets Manager via IRSA** -- keyless reads through a referenced ServiceAccount's IAM role binding. Start from the **AWS Secrets Manager IRSA** preset.

**GCP Secret Manager via Workload Identity** -- keyless reads through a GKE Workload Identity binding. Start from the **GCP Secret Manager Workload Identity** preset.

**Azure Key Vault via Workload Identity** -- keyless reads through an AKS federated credential. Start from the **Azure Key Vault Workload Identity** preset.

**Vault with Kubernetes auth** -- the ServiceAccount token is exchanged for a Vault token; no Vault credential stored on the cluster. Start from the **Vault Kubernetes Auth** preset.

## Works With

- **Kubernetes External Secrets Operator** -- must be on the cluster first; cluster-scoped stores reference its namespace output as their secrets home.
- **Kubernetes External Secret** -- declares each secret to sync through this store (kind ClusterSecretStore).
- **Kubernetes Secret Store** -- the namespaced twin, for connections that belong to one team.
