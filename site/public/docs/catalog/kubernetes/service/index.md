...
- [KubernetesDeployment](/docs/catalog/kubernetes/deployment) — workload kinds already create a Service for their own pods; select their pods here with `app: <workload-metadata-name>` when you need additional exposure
...
- [KubernetesIngress](/docs/catalog/kubernetes/ingress) — L7 routing (hostnames, paths, TLS) in front of a Service backend
