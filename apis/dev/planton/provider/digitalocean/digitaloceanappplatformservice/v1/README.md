...
The **DigitalOcean App Platform Service** API resource provides a simplified, declarative interface for deploying containerized applications and services on DigitalOcean's fully-managed Platform-as-a-Service (PaaS). Following Planton's 80/20 principle, this API exposes only the essential configuration fields needed for the vast majority of deployments, abstracting away platform complexity while maintaining production-grade capabilities.
...
This API resource integrates with Planton's unified infrastructure framework, using a Kubernetes-style manifest format (`apiVersion`, `kind`, `metadata`, `spec`) for consistency across cloud providers.
...
- **Foreign Key References**: Reference other Planton resources (DNS zones, databases, container registries) using declarative field paths
...
- **Planton Documentation**: [docs.planton.ai](https://docs.planton.ai)
...
- **Planton GitHub**: [github.com/plantonhq/planton](https://github.com/plantonhq/planton)
