...
Planton's abstraction consolidates what would otherwise be a multi-resource Terraform/Pulumi deployment into a single declarative manifest with proto-validated fields, foreign key references to other Planton components, and sensible defaults that cover the common case while exposing the full breadth of configuration when needed.
...
This is the problem Planton solves: collapse four resources into one manifest with one `planton apply` command.
...
Planton collapses the four-resource deployment into a single YAML manifest:
...
One file. One `planton apply`. The IaC modules handle resource creation ordering, ID type conversions, and dependency wiring.
...
- Deprecated `target` inline list on the load balancer resource — replaced by separate target resources, which is what Planton uses.
