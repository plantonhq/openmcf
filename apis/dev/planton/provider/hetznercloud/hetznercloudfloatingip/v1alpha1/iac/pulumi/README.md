# HetznerCloudFloatingIp Pulumi Module

Pulumi (Go) IaC module for allocating reassignable public IP addresses in Hetzner Cloud with optional server assignment and optional reverse DNS.
...
│   ├── floating_ip.go # Floating IP creation, optional server assignment, conditional rDNS; output exports
...
- `hcloud.FloatingIp` — Allocates an IPv4 address or IPv6 /64 block with optional server assignment, labels, and protection settings
...
```bash
# Build
bazel build //apis/dev/planton/provider/hetznercloud/hetznercloudfloatingip/v1alpha1/iac/pulumi:pulumi
