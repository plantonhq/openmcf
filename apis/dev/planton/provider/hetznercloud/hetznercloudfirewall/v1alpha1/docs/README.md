...
**Verdict:** Excellent for Go/TypeScript teams and dynamic rule generation. Planton uses Pulumi (Go) internally for its IaC modules.
...
The key differentiator for Planton is **proto-level validation with CEL rules**. Terraform and Pulumi catch errors at the Hetzner Cloud API level (during `apply`/`up`). Planton catches three classes of errors at manifest validation time, before any API call:
...
The Hetzner Cloud firewall API has 4 user-controllable attributes: `name`, `rules`, `labels`, and `apply_to`. Planton's `HetznerCloudFirewallSpec` exposes 1 field: `rules`.
