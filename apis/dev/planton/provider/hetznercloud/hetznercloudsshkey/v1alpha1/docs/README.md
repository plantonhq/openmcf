...
Planton exposes exactly one spec field — `publicKey` — because the SSH key resource has exactly one user-controlled attribute. The name comes from `metadata.name`, labels are computed from metadata, and everything else (fingerprint, numeric ID) is a computed output. This is one of the simplest possible Planton components, and that simplicity is intentional.
...
**Verdict:** Production-grade for teams already using Terraform. The standard choice before Planton.
...
| **Planton** | **Yes** | **Yes** | **Via backend** | **Via VCS** | **Yes** |

The key differentiator for Planton is not the state management (Terraform and Pulumi handle that) — it is the **standardized manifest format** and **output referencing** that makes SSH keys composable with servers, infra charts, and other components without writing custom glue code.
...
The Hetzner Cloud SSH key API has 3 user-controllable attributes: `name`, `public_key`, and `labels`. Planton's `HetznerCloudSshKeySpec` exposes 1 field: `publicKey`.
...
This is not a reduction in capability — it is a shift of responsibility. The user still controls all three attributes, but `name` and `labels` are managed through the metadata system that is consistent across every Planton component.
