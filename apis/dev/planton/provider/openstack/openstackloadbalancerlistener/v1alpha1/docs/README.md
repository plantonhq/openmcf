...
- **L7 policies**: Complex routing rules (not yet in Planton scope)
...
The Terraform `openstack_lb_listener_v2` resource exposes 16+ schema fields. Planton's `OpenStackLoadBalancerListenerSpec` exposes 11 fields -- the ones that cover 95%+ of real-world use cases:
...
- For multiple domains on one listener, consider SNI (not yet in Planton scope)
