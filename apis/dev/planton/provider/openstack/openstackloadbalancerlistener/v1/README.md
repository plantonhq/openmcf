...
Provision and manage Octavia listeners in OpenStack using Planton's unified API.
...
Reference a load balancer managed by Planton using `value_from`:
...
```bash
planton apply --manifest listener.yaml \
