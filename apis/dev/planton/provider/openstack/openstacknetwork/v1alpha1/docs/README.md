...
The network is the root of the OpenStack networking dependency tree, and Planton's `OpenStackNetwork` component reflects this: it has no foreign key dependencies, but nearly every other OpenStack component references it.
...
**Verdict:** Industry standard for infrastructure automation. Good for teams comfortable with HCL. Lacks the unified API and multi-cloud abstraction that Planton provides.
...
**Verdict:** Excellent for teams with software engineering backgrounds. Planton uses Pulumi internally for its Go-native IaC modules.
...
| **Planton** | **Yes** | **Yes** | **Yes** | **Yes** | **Low** |

## The Planton Approach

Planton wraps both Terraform and Pulumi behind a unified, Kubernetes-style API:
...
The Terraform `openstack_networking_network_v2` resource exposes 19 arguments. Planton's `OpenStackNetworkSpec` exposes 9 fields — the ones that cover 95%+ of real-world use cases:
...
**Network name from `metadata.name`:** The Planton resource name IS the OpenStack network name. This is consistent across all OpenStack components and mirrors how users think about resources.
...
- Use `metadata.labels` for Planton-level metadata; use `spec.tags` for OpenStack-level tags
