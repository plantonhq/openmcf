...
type Locals struct {
	...
	OpenStackLoadBalancerMonitor *openstackloadbalancermonitorv1alpha1.OpenStackLoadBalancerMonitor
	// PoolId is the resolved pool ID from the StringValueOrRef.
	PoolId string
}

// initializeLocals copies fields from the stack input into Locals.
func initializeLocals(_ *pulumi.Context, stackInput *openstackloadbalancermonitorv1alpha1.OpenStackLoadBalancerMonitorStackInput) *Locals {
	locals := &Locals{}
