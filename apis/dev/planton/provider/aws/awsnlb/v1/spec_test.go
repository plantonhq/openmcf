...
func TestAwsNlbSpec(t *testing.T) {
	...
	ginkgo.RunSpecs(t, "AwsNlbSpec Validation Suite")
}
...
func minimalListener(name string, port int32) *AwsNlbListener {
	return &AwsNlbListener{
		Name:     name,
		Port:     port,
		Protocol: "TCP",
		TargetGroup: &AwsNlbTargetGroup{
			Port:     port,
			Protocol: "TCP",
		},
	}
...
func minimalSubnetMapping(subnetId string) *AwsNlbSubnetMapping {
	return &AwsNlbSubnetMapping{
		SubnetId: strRef(subnetId),
	}
}

var _ = ginkgo.Describe("AwsNlbSpec validations", func() {
	var spec *AwsNlbSpec
	...
	ginkgo.It("accepts multiple subnet mappings across AZs", func() {
		spec.SubnetMappings = []*AwsNlbSubnetMapping{
	...
	ginkgo.It("accepts target group with all options", func() {
		spec.Listeners[0].TargetGroup = &AwsNlbTargetGroup{
			Port:                       8080,
			Protocol:                   "TCP",
			TargetType:                 "ip",
	...
	ginkgo.It("accepts target group with HTTP health check", func() {
		spec.Listeners[0].TargetGroup.HealthCheck = &AwsNlbHealthCheck{
			Protocol:           "HTTP",
			Port:               "traffic-port",
			Path:               "/health",
			HealthyThreshold:   5,
