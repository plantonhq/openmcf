...
This document maps the OpenSearch deployment landscape: cluster architecture, data tiers, networking, security, storage, and how Planton abstracts complexity while preserving the power needed for production workloads.
...
**Index State Management (ISM) policies** automate this lifecycle. An ISM policy can automatically move an index from hot to warm after 14 days, and from warm to cold after 90 days. This isn't configured in the Planton spec (it's an OpenSearch-level setting), but enabling warm and cold tiers in the spec makes ISM policies possible.
...
Several spec fields accept `StringValueOrRef`, enabling runtime composition with other Planton resources:
