...
|---|---|---|
| `status.outputs.clusterIdentifier` | `string` | The cluster identifier (e.g. "orders-db"). |
...
| `status.outputs.instanceEndpoints` | `[]string` | Per-instance endpoints of the cluster's folded instances, ordered as declared in spec.instances. Empty for Aurora Serverless v1 and Multi-AZ RDS clusters, where AWS owns the compute. |
