...
Planton's `AwsCodePipeline` component exposes the full pipeline definition as a single declarative manifest: stages, actions, artifact stores, triggers, and variables. The result is a portable, reviewable YAML document that captures your entire release workflow.
