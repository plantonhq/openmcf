...
The `AwsEcrRepo` API in Planton follows the principle of **secure defaults with escape hatches for advanced use cases**.
...
Planton's API design prioritizes:
...
The API intentionally stays focused on repository provisioning fundamentals. For advanced features like lifecycle policies, repository policies, and scanning configuration, Planton follows the philosophy of composability—these can be managed through companion configurations or post-deployment automation.

### When to Use Planton's AwsEcrRepo
...
Planton's `AwsEcrRepo` API embraces these principles, providing a minimal, secure-by-default interface that works seamlessly in multi-cloud, GitOps-driven environments. The true power lies not in the number of configuration options, but in making the right choices the easy choices.
