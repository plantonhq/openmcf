...
The `AwsS3ObjectSet` component addresses the gap between "bucket exists" and "bucket has the right content." It provides a declarative way to manage a collection of S3 objects within a single bucket, tightly integrated with Planton's resource graph via foreign key references to `AwsS3Bucket`.
...
Planton's `AwsS3ObjectSet` takes the IaC approach further by providing:
...
- **Planton**: Full state via Terraform/Pulumi, foreign key references, batch objects, tag inheritance. Minimal configuration for common patterns.
...
- Reduces the number of Planton resources to manage
