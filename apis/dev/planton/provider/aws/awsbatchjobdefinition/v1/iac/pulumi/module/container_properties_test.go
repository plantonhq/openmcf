...
func TestContainerProperties_Minimal(t *testing.T) {
	spec := &awsbatchjobdefinitionv1.AwsBatchJobDefinitionSpec{
		Region: "us-west-2",
		Container: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionContainer{
...
func TestContainerProperties_FullSurface(t *testing.T) {
	spec := &awsbatchjobdefinitionv1.AwsBatchJobDefinitionSpec{
		...
		Container: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionContainer{
			...
			LogConfiguration: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionLogConfiguration{
				LogDriver:     "awslogs",
			...
			MountPoints: []*awsbatchjobdefinitionv1.AwsBatchJobDefinitionMountPoint{
				{SourceVolume: "shared", ContainerPath: "/data", ReadOnly: true},
			},
			Volumes: []*awsbatchjobdefinitionv1.AwsBatchJobDefinitionVolume{
				{
					Name: "shared",
					Efs: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionEfsVolume{
			...
			Ulimits: []*awsbatchjobdefinitionv1.AwsBatchJobDefinitionUlimit{
				{Name: "nofile", SoftLimit: 8192, HardLimit: 65535},
			},
			LinuxParameters: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionLinuxParameters{
				InitProcessEnabled:  true,
				SharedMemorySizeMib: 256,
				Tmpfs: []*awsbatchjobdefinitionv1.AwsBatchJobDefinitionTmpfs{
					{ContainerPath: "/tmp/scratch", SizeMib: 128, MountOptions: []string{"noexec"}},
				},
				Devices: []*awsbatchjobdefinitionv1.AwsBatchJobDefinitionDevice{
...
func TestContainerProperties_FargateKnobs(t *testing.T) {
	spec := &awsbatchjobdefinitionv1.AwsBatchJobDefinitionSpec{
		...
		Container: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionContainer{
			...
			RuntimePlatform: &awsbatchjobdefinitionv1.AwsBatchJobDefinitionRuntimePlatform{
