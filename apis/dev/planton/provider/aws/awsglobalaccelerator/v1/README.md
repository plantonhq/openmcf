...
The **AwsGlobalAccelerator** resource provides a standardized way to provision and manage AWS Global Accelerator through Planton. It creates a Layer 4 anycast networking service that routes traffic through the AWS global backbone to optimal regional endpoints based on health, geography, and routing policies.

## What Gets Created

When you deploy an AwsGlobalAccelerator resource, Planton provisions the complete resource hierarchy:
...
```shell
planton pulumi up --manifest accelerator.yaml --stack <stack-name>
```
