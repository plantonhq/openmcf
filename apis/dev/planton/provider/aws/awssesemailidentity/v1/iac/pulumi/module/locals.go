...
type Locals struct {
	AwsSesEmailIdentity *awssesemailidentityv1.AwsSesEmailIdentity
	AwsTags             map[string]string
	// Dkim is emitted only when the manifest configures signing.
	Dkim *awssesemailidentityv1.AwsSesEmailIdentityDkimSigning
	// Byodkim is selected by the key/selector pair.
	Byodkim bool
	// Policies maps policy name -> spec entry for per-name satellites.
	Policies map[string]*awssesemailidentityv1.AwsSesEmailIdentityPolicy
}

func initializeLocals(_ *pulumi.Context, stackInput *awssesemailidentityv1.AwsSesEmailIdentityStackInput) *Locals {
	locals := &Locals{}
	...
	locals.Policies = make(map[string]*awssesemailidentityv1.AwsSesEmailIdentityPolicy)
