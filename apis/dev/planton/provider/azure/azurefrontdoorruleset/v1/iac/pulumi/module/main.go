...
func Resources(ctx *pulumi.Context, stackInput *azurefrontdoorrulesetv1.AzureFrontDoorRuleSetStackInput) error {
	...
	for _, rule := range spec.Rules {
		...
		if rule.BehaviorOnMatch != azurefrontdoorrulesetv1.AzureFrontDoorRuleBehaviorOnMatch_azure_front_door_rule_behavior_on_match_unspecified {
...
func buildRuleActions(actions *azurefrontdoorrulesetv1.AzureFrontDoorRuleActions) *cdn.FrontdoorRuleActionsArgs {
	...
	if actions.UrlRedirect != nil {
		...
		if actions.UrlRedirect.RedirectProtocol != azurefrontdoorrulesetv1.AzureFrontDoorRuleForwardingProtocol_azure_front_door_rule_forwarding_protocol_unspecified {
	...
	if actions.RouteConfigurationOverride != nil {
		...
		if actions.RouteConfigurationOverride.QueryStringCachingBehavior != azurefrontdoorrulesetv1.AzureFrontDoorRuleQueryStringCachingBehavior_azure_front_door_rule_query_string_caching_behavior_unspecified {
...
func buildRuleConditions(conditions *azurefrontdoorrulesetv1.AzureFrontDoorRuleConditions) *cdn.FrontdoorRuleConditionsArgs {
...
// proto defaults).
func addressOperator(operator azurefrontdoorrulesetv1.AzureFrontDoorRuleOperator) string {
	if operator == azurefrontdoorrulesetv1.AzureFrontDoorRuleOperator_azure_front_door_rule_operator_unspecified {
		return "IPMatch"
	}
...
func transformArray(transforms []azurefrontdoorrulesetv1.AzureFrontDoorRuleTransform) pulumi.StringArrayInput {
	if len(transforms) == 0 {
		return nil
	}
