locals {
  ...
  base_tags = {
    ...
    "resource_kind" = "azure_container_app_environment_managed_certificate"
  ...
  # The validation enum arrives as the proto value name; both values match
  # Azure's wire vocabulary verbatim. Unspecified deploys HTTP -- Azure's
  # own default -- explicitly, keeping both engines identical.
  domain_control_validation = (
    var.spec.domain_control_validation != null && var.spec.domain_control_validation != ""
    ? var.spec.domain_control_validation
    : "HTTP"
  )
}
