...
- [AzureVirtualNetwork](/docs/catalog/azure/azurevirtualnetwork) — provides the parent Virtual Network that contains this subnet
- [AzureRouteTable](/docs/catalog/azure/azureroutetable) — attaches via `routeTableId` to steer the subnet's egress
- [AzureNetworkSecurityGroup](/docs/catalog/azure/azurenetworksecuritygroup) — attaches via `networkSecurityGroupId` to filter the subnet's traffic
- [AzureNatGateway](/docs/catalog/azure/azurenatgateway) — attaches via `natGatewayId` to own the subnet's outbound connectivity
