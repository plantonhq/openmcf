...
// validateRoleDefinitionId rejects a role_definition_id that is not a
// Cosmos DB SQL role definition resource ID. The Terraform provider
// performs this check at plan time (ValidateSqlRoleDefinitionID); the
// Pulumi SDK carries no equivalent, so the module enforces the same
// contract to keep both engines failing loudly and early on a malformed
// literal instead of surfacing a late ARM error.
func validateRoleDefinitionId(roleDefinitionId string) error {
	parts := strings.Split(roleDefinitionId, "/sqlRoleDefinitions/")
	if len(parts) != 2 || parts[1] == "" || strings.Contains(parts[1], "/") ||
		!strings.Contains(parts[0], "/databaseAccounts/") {
		return errors.Errorf("role_definition_id %q is not a Cosmos DB SQL role definition resource id (expected .../databaseAccounts/{account}/sqlRoleDefinitions/{guid})", roleDefinitionId)
	}
	return nil
}
