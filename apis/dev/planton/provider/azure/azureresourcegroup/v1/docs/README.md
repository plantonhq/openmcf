...
## 3. Why Planton Models Resource Groups

### The Composability Argument

Planton's power comes from `StringValueOrRef` -- the ability for one resource to
...
1. `name` (required)
2. `location` (required)
3. `tags` (optional, handled by Planton metadata)
...
- **Resource Groups**: The primary IaC target for application infrastructure. This
  is where Planton operates.

## 6. Design Decisions

### Why `name` Is a Spec Field (Not Derived from Metadata)

In most Planton resources, the resource name is derived from `metadata.name`.
...
1. Resource group names follow enterprise naming conventions that may differ
   from the Planton metadata name
2. Multiple Planton resources may reference the same resource group by name
3. The resource group name is an Azure-level identifier that appears in Azure
   Portal, CLI output, and ARM resource IDs

The `metadata.name` serves as the Planton-level identifier for DAG wiring,
while `spec.name` is the Azure-level resource group name.

### Why No Tags in Spec

Tags are handled automatically via Planton metadata (name, org, env) and
propagated to Azure tags in the IaC modules. This follows the established
pattern across all Planton Azure resources.
...
- Tags the resource group with Planton metadata
