...
func Resources(ctx *pulumi.Context, stackInput *azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerStackInput) error {
	...
	isDefaultMode := true
	if spec.CreateMode != azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerCreateMode_azure_postgresql_flexible_server_create_mode_unspecified {
		serverArgs.CreateMode = pulumi.String(createModeStrings[spec.CreateMode])
		isDefaultMode = spec.CreateMode == azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerCreateMode_DEFAULT
	}
	...
	if spec.ReplicationRole == azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerReplicationRole_NONE {
	...
	if spec.StorageTier != azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerStorageTier_azure_postgresql_flexible_server_storage_tier_unspecified {
	...
	hasSystemIdentity := spec.Identity != nil &&
		(spec.Identity.Type == azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerIdentityType_SYSTEM_ASSIGNED ||
			spec.Identity.Type == azurepostgresqlflexibleserverv1.AzurePostgresqlFlexibleServerIdentityType_SYSTEM_AND_USER_ASSIGNED)
