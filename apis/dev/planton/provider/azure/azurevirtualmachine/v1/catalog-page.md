...
- [AzureNetworkInterface](/docs/catalog/azure/azurenetworkinterface) — the VM's network presence: subnet placement, public IPs, NSG filtering, accelerated networking
- [AzureManagedDisk](/docs/catalog/azure/azuremanageddisk) — first-class data volumes that outlive the VM, and the golden-disk boot source
- [AzureUserAssignedIdentity](/docs/catalog/azure/azureuserassignedidentity) — shareable managed identities attached via `identity.identityIds`
- [AzureRoleAssignment](/docs/catalog/azure/azureroleassignment) — grants against the VM's system-assigned principal or its user-assigned identities
- [AzureKeyVault](/docs/catalog/azure/azurekeyvault) — certificates installed onto the VM at provisioning time
