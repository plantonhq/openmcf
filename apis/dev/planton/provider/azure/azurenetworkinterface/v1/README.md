...
- **Association Resources**: NSG and application-security-group memberships are realized as separate association resources (Azure's own model), so filtering can change without touching the NIC
...
- **Load-Balancer Backend Membership**: Backend-pool membership is expressed NIC-side in Azure's model; it will become spec surface once the load balancer exports per-pool IDs to reference
