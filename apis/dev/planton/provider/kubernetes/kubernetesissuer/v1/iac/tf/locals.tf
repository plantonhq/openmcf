# Computed values for the KubernetesIssuer module.
#
...
locals {
  ...
  labels = merge(concat(
    [{
      ...
      "planton.ai/resource-kind" = "KubernetesIssuer"
    }],
  ...
  # ---- ACME ------------------------------------------------------------
  acme_account_key_secret_name = local.acme != null ? "${local.issuer_name}-acme-account-key" : ""

  acme_eab_secret_name = local.acme != null && try(local.acme.external_account_binding, null) != null ? "${local.issuer_name}-acme-eab" : ""
      ...
      for k, v in {
        ...
        cloudflare = try(s.dns01.cloudflare, null) == null ? null : {
          for ck, cv in {
            ...
            apiTokenSecretRef = try(s.dns01.cloudflare.api_token, null) != null ? { name = "${local.issuer_name}-solver${i}-cloudflare", key = "api-token" } : null
            apiKeySecretRef   = try(s.dns01.cloudflare.api_key, null) != null ? { name = "${local.issuer_name}-solver${i}-cloudflare", key = "api-key" } : null
          } : ck => cv if cv != null
        }
        ...
        route53 = try(s.dns01.route53, null) == null ? null : {
          for rk, rv in {
            ...
            secretAccessKeySecretRef = try(s.dns01.route53.static_credentials, null) != null ? { name = "${local.issuer_name}-solver${i}-route53", key = "secret-access-key" } : null
        ...
        azureDNS = try(s.dns01.azure_dns, null) == null ? null : {
          for ak, av in {
            ...
            clientSecretSecretRef = try(s.dns01.azure_dns.client_secret, "") != "" ? { name = "${local.issuer_name}-solver${i}-azure-dns", key = "client-secret" } : null
        ...
        cloudDNS = try(s.dns01.gcp_cloud_dns, null) == null ? null : {
          for gk, gv in {
            ...
            serviceAccountSecretRef = try(s.dns01.gcp_cloud_dns.service_account_key_json, "") != "" ? { name = "${local.issuer_name}-solver${i}-clouddns", key = "key.json" } : null
          } : gk => gv if gv != null
        }

        digitalocean = try(s.dns01.digitalocean, null) == null ? null : {
          tokenSecretRef = { name = "${local.issuer_name}-solver${i}-digitalocean", key = "access-token" }
        }
        ...
        rfc2136 = try(s.dns01.rfc2136, null) == null ? null : {
          for fk, fv in {
            ...
            tsigSecretSecretRef = try(s.dns01.rfc2136.tsig_key_name, "") != "" ? { name = "${local.issuer_name}-solver${i}-rfc2136", key = "tsig-secret" } : null
        ...
        acmeDNS = try(s.dns01.acme_dns, null) == null ? null : {
          ...
          accountSecretRef = { name = "${local.issuer_name}-solver${i}-acme-dns", key = "acmedns.json" }
        }

        akamai = try(s.dns01.akamai, null) == null ? null : {
          serviceConsumerDomain = s.dns01.akamai.service_consumer_domain
          clientTokenSecretRef  = { name = "${local.issuer_name}-solver${i}-akamai", key = "client-token" }
          clientSecretSecretRef = { name = "${local.issuer_name}-solver${i}-akamai", key = "client-secret" }
          accessTokenSecretRef  = { name = "${local.issuer_name}-solver${i}-akamai", key = "access-token" }
        }
  ...
  solver_credential_secrets = merge(concat([{}], [
    for i, s in try(local.acme.solvers, []) : merge(
      try(s.dns01.cloudflare.api_token, null) != null ? {
        "${local.issuer_name}-solver${i}-cloudflare" = { "api-token" = s.dns01.cloudflare.api_token.token }
      } : {},
      try(s.dns01.cloudflare.api_key, null) != null ? {
        "${local.issuer_name}-solver${i}-cloudflare" = { "api-key" = s.dns01.cloudflare.api_key.key }
      } : {},
      try(s.dns01.route53.static_credentials, null) != null ? {
        "${local.issuer_name}-solver${i}-route53" = { "secret-access-key" = s.dns01.route53.static_credentials.secret_access_key }
      } : {},
      try(s.dns01.azure_dns.client_secret, "") != "" ? {
        "${local.issuer_name}-solver${i}-azure-dns" = { "client-secret" = s.dns01.azure_dns.client_secret }
      } : {},
      try(s.dns01.gcp_cloud_dns.service_account_key_json, "") != "" ? {
        "${local.issuer_name}-solver${i}-clouddns" = { "key.json" = s.dns01.gcp_cloud_dns.service_account_key_json }
      } : {},
      try(s.dns01.digitalocean, null) != null ? {
        "${local.issuer_name}-solver${i}-digitalocean" = { "access-token" = s.dns01.digitalocean.token }
      } : {},
      try(s.dns01.rfc2136.tsig_key_name, "") != "" ? {
        "${local.issuer_name}-solver${i}-rfc2136" = { "tsig-secret" = s.dns01.rfc2136.tsig_secret }
      } : {},
      try(s.dns01.acme_dns, null) != null ? {
        "${local.issuer_name}-solver${i}-acme-dns" = { "acmedns.json" = s.dns01.acme_dns.account_json }
      } : {},
      try(s.dns01.akamai, null) != null ? {
        "${local.issuer_name}-solver${i}-akamai" = {
  ...
  credential_secrets = merge(concat(
    ...
    (local.vault != null && try(local.vault.token_auth, null) != null) ? [{
      "${local.issuer_name}-vault-token" = { "token" = local.vault.token_auth.token }
    }] : [],
    (local.vault != null && try(local.vault.app_role_auth, null) != null) ? [{
      "${local.issuer_name}-vault-approle" = { "secret-id" = local.vault.app_role_auth.secret_id }
    }] : []
  )...)
          ...
          auth = {
            for auth_k, auth_v in {
              tokenSecretRef = try(local.vault.token_auth, null) == null ? null : {
                name = "${local.issuer_name}-vault-token", key = "token"
              }
              appRole = try(local.vault.app_role_auth, null) == null ? null : {
                ...
                secretRef = { name = "${local.issuer_name}-vault-approle", key = "secret-id" }
              }
