# Provider requirements for the KubernetesKafkaConnect module.
#
# kubectl (alekc/kubectl) applies the KafkaConnect custom resource: unlike
...
# Connect cluster can be planned before the Strimzi operator's CRDs exist
# (single-run infra charts, offline plan proofs). The hashicorp kubernetes
# provider materializes the namespace and the metrics ConfigMap.
#
