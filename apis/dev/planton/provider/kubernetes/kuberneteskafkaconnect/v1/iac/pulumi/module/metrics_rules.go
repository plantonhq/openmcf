...
// ConfigMap when spec.metrics.enabled — the canonical Strimzi Connect rule
// set (kafkaConnectMetricsRules below) under the key the KafkaConnect CR's
// metricsConfig points at. Returns nil when metrics are disabled.
// Terraform equivalent: kubernetes_config_map_v1 with count.
	...
	createdConfigMap, err := kubernetescorev1.NewConfigMap(ctx, locals.MetricsConfigMapName,
		&kubernetescorev1.ConfigMapArgs{
			...
			Data: pulumi.StringMap{
				vars.MetricsConfigKey: pulumi.String(kafkaConnectMetricsRules),
			},
		}, append([]pulumi.ResourceOption{pulumi.Provider(kubernetesProvider)}, dependencies...)...)
	if err != nil {
		return nil, errors.Wrap(err, "failed to create kafka connect metrics ConfigMap")
	}

	return createdConfigMap, nil
}

// kafkaConnectMetricsRules is the canonical Strimzi JMX Prometheus
// Exporter rule set for Kafka Connect workers, copied VERBATIM from the
// pinned upstream example (examples/metrics/kafka-connect-metrics.yaml,
// ConfigMap key metrics-config.yml). The Terraform module embeds the SAME
// content (metrics_rules.tf) — the two must stay byte-identical so both
// engines render one ConfigMap shape.
const kafkaConnectMetricsRules = `# Inspired by kafka-connect rules
