...
- [KubernetesDeployment](/docs/catalog/kubernetes/kubernetesdeployment) — for its OWN pods, use the built-in `availability.pod_disruption_budget` block instead; its pods carry the `app: <workload-name>` label this kind's selector targets for composition
- [KubernetesHorizontalPodAutoscaler](/docs/catalog/kubernetes/kuberneteshorizontalpodautoscaler) — the scaling counterpart; a budget bounds how fast the fleet can shrink during drains while the autoscaler owns how large it runs
