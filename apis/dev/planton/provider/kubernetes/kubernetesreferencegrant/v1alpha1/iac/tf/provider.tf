# Provider requirements for the KubernetesReferenceGrant module.
#
# kubectl (alekc/kubectl) applies the ReferenceGrant CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the ReferenceGrant can be planned before the
