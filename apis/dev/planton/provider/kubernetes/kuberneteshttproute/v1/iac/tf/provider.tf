# Provider requirements for the KubernetesHttpRoute module.
#
# kubectl (alekc/kubectl) applies the HTTPRoute CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the HTTPRoute can be planned before the
