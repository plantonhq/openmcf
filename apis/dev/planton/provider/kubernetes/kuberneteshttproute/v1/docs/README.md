...
`KubernetesHttpRoute` brings that resource into Planton as a first-class
...
boundary Planton's resource graph and InfraChart composition reinforce.
...
The Planton spec flattens the upstream `HTTPRouteSpec` after the standard
Kubernetes envelope:
...
Planton that the CRD would reject:
...
## Why HTTPRoute Is a First-Class Planton Component

Without a first-class component, HTTP routing on the Gateway API requires raw
`KubernetesManifest` YAML, which forfeits the entire Planton value proposition:
...
Pulumi resource Planton provisions with. The following upstream fields are
...
`KubernetesHttpRoute` closes the most important gap in Planton's networking
...
routing is a first-class Planton experience rather than raw YAML.
