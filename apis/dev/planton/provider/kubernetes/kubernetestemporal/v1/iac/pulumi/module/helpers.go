...
// component image shape ({repository, tag} — the chart's images are the
// COMBINED form: the repository value may carry a registry host). Nil
// when nothing is declared. Pull secrets are collected separately into
// the chart's global imagePullSecrets list.
...
// joinHostPort renders the chart's REQUIRED connectAddr form (host:port —
// its env template parses SQL_HOST/SQL_PORT out of it).
func joinHostPort(host string, port int32) string {
	return fmt.Sprintf("%s:%d", host, port)
}

// mergeInto copies every entry of src into dst (shallow — used for the
// per-component scheduling blocks).
