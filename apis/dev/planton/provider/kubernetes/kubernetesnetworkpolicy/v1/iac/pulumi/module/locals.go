...
type Locals struct {
	...
	Spec        *kubernetesnetworkpolicyv1.KubernetesNetworkPolicySpec
	Namespace   string
	Name        string
...
func initializeLocals(ctx *pulumi.Context, stackInput *kubernetesnetworkpolicyv1.KubernetesNetworkPolicyStackInput) *Locals {
...
func resolvePolicyTypes(spec *kubernetesnetworkpolicyv1.KubernetesNetworkPolicySpec) []string {
	if len(spec.GetPolicyTypes()) > 0 {
		types := make([]string, 0, len(spec.GetPolicyTypes()))
		for _, t := range spec.GetPolicyTypes() {
			if t == kubernetesnetworkpolicyv1.KubernetesNetworkPolicySpec_egress {
