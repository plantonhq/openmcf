...
## Why GatewayClass Is a First-Class Planton Component
...
the Gateway API admission webhook at apply time. Enforcing it inside the Planton
...
Planton foreign keys (`StringValueOrRef`) are for single-value identifiers or
outputs of *other Planton resources*. `parametersRef` is a multi-field reference
(group/kind/name/namespace) to an arbitrary Kubernetes object that is usually not
an Planton resource (a ConfigMap, or an implementation CRD like `EnvoyProxy`).
...
this is why the Planton field preserves the exact upstream string rather than an
enum.
...
Modeling it as a first-class Planton component yields fleet-wide visibility,
...
Planton Gateway API networking layer.
