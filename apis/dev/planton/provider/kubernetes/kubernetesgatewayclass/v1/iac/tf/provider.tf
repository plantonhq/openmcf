# Provider requirements for the KubernetesGatewayClass module.
#
# kubectl (alekc/kubectl) applies the GatewayClass CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the GatewayClass can be planned before the
