...
- `AtlasMongodb`
- `SnowflakeDatabase`
- `AwsAlb` -- 200–399: AWS resources AwsSubnet is a prerequisite because an ALB requires at least two subnets in different availability zones -- the spec's subnet references must resolve before the load balancer can be created.
- `AwsCertManagerCert`
- `AwsCloudFront`
- `AwsDynamodb`
- `AwsEcrRepo`
...
- `AwsSesEmailIdentity`
- `AzureResourceGroup` -- 400–599: Azure resources
...
- `AzureIpGroup`
- `GcpArtifactRegistryRepo` -- 600–799: GCP resources
- `GcpTargetHttpsProxy`
- `GcpCloudFunction`
- `GcpCloudRun`
- `GcpCloudSql`
...
- `GcpWorkloadIdentityPool` -- 701–709: IAM/identity family (overflow block; the 600–622 foundation/security sub-band is fully allocated)
- `GcpWorkloadIdentityPoolProvider`
- `GcpServiceAccountIamMember`
- `GcpGlobalForwardingRule` -- 710–719: networking/load-balancer family (overflow block; the 623–629 LB sub-band is fully allocated)
- `GcpSslPolicy`
- `GcpSslCertificate`
...
- `GcpCloudRunJob` -- 720–729: GCP serverless overflow
- `GcpServerlessVpcConnector`
- `GcpComputeDisk` -- 730–739: GCP compute overflow (the 600–622 foundation sub-band that holds GcpComputeInstance is fully allocated)
- `KubernetesNamespace` -- 800–999: Kubernetes resources, organized in family sub-bands (830–869 also hosts CNI/autoscaling/DR addons; 930–949 hosts analytics & ML; 990–999 reserved for growth) 800–829: Kubernetes building blocks (core API primitives)
...
- `KubernetesCertManager` -- 830–869: Kubernetes foundation addons (certs, DNS, secrets, ingress, Gateway API, mesh, CNI/autoscaling/DR)
...
- `KubernetesIstio`
- `KubernetesDestinationRule` -- Istio API components (mesh traffic policy, security, telemetry). The seven typed resources below (853–859) require the Istio CRDs on the cluster, provided by the lightweight CRDs-only KubernetesIstioBaseCrds (851) — NOT the full mesh KubernetesIstio (852).
...
- `KubernetesVelero`
- `KubernetesKubePrometheusStack` -- 870–889: Kubernetes observability
- `KubernetesGrafana`
...
- `KubernetesKyverno` -- 880–899: Kubernetes security, policy, and identity
...
- `KubernetesCloudNativePgOperator` -- 900–929: Kubernetes data platforms
- `KubernetesPostgres`
- `KubernetesValkey`
...
- `KubernetesArgocd` -- 950–969: Kubernetes GitOps and CI/CD
...
- `KubernetesHarbor`
- `KubernetesJenkins`
- `KubernetesTemporal` -- 970–989: Kubernetes app platforms KubernetesPostgres is a prerequisite because the recommended (and E2E-proven) database composition backs Temporal's default and visibility stores with a CloudNativePG cluster.
- `KubernetesNats`
- `KubernetesLocust`
- `DigitalOceanAppPlatformService` -- 1200–1499: DigitalOcean resources
- `DigitalOceanBucket`
...
- `CivoBucket` -- 1500–1799: Civo resources
- `CivoCertificate`
- `CivoComputeInstance`
- `CivoDatabase`
- `CivoDnsZone`
...
- `CivoVpc`
- `CivoDnsRecord`
- `CloudflareDnsZone` -- 1800–2099: Cloudflare resources
...
- `Auth0Connection` -- 2100–2299: Auth0 resources
- `Auth0Client`
- `Auth0EventStream`
- `Auth0ResourceServer`
- `Auth0Action`
- `Auth0Role`
- `OpenFgaStore` -- 2300–2499: OpenFGA resources Note: OpenFGA is Terraform-only - there is no Pulumi provider available. Pulumi modules for OpenFGA resources are pass-through placeholders.
- `OpenFgaAuthorizationModel`
- `OpenFgaRelationshipTuple`
- `OpenStackKeypair` -- 2500–2799: OpenStack resources
- `OpenStackNetwork`
- `OpenStackSubnet`
- `OpenStackRouter`
