# Provider requirements for the KubernetesTcpRoute module.
#
# kubectl (alekc/kubectl) applies the TCPRoute CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the TCPRoute can be planned before the
