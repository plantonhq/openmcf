# Provider requirements for the KubernetesClusterSecretStore module.
#
# kubectl (alekc/kubectl) applies the ClusterSecretStore CR: unlike the
# hashicorp kubernetes provider's kubernetes_manifest resource,
# kubectl_manifest needs no cluster connection at plan time, so the store
...
# provider materializes the credential Secrets.
#
