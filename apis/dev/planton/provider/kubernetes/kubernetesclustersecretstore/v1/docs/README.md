# KubernetesClusterSecretStore: Research and Design

## Introduction

A ClusterSecretStore is the External Secrets Operator (ESO) custom resource
that represents one backend connection serving ExternalSecrets in EVERY
namespace of a cluster. This component creates one ClusterSecretStore, named
after the resource (`metadata.name`), plus the credential Secret its
...
The Planton kinds mirror this split exactly: KubernetesClusterSecretStore
and KubernetesSecretStore share one `ExternalSecretsStoreConfig` proto and
one Pulumi spec builder, so the twins are structurally incapable of
...
## The Grain: One Store, Named After the Resource

The store's identity is `metadata.name` — the same convention every other
kind follows. ExternalSecrets select it by name
(`secretStoreRef.name` with `kind: ClusterSecretStore`), and the
`store_name` stack output carries that handle for composition. Nothing
constrains a store to one consumer: a single platform-wide store serves
every team, optionally fenced with `conditions`.
...
1. **Keyless via a referenced ServiceAccount** — `service_account_name` is a
   ...
   different stores can carry different identities. Cluster scope has no
   home namespace, so `service_account_namespace` is required here (the
   builder defaults it to the secrets namespace).
...
3. **Declared static credentials** — sensitive spec values are materialized
   ...
   store's secrets namespace; the CR references that Secret. Exactly-one
   contracts reject mixing postures (e.g. a ServiceAccount reference AND
   static keys), and paired-field contracts reject half-credentials.

## Credential Model Mechanics

Cluster-scoped stores read their referenced Secrets from an EXPLICIT
namespace — there is no "own namespace" to default to — so
`spec.secrets_namespace` is required, conventionally the operator's install
namespace (its FK default is KubernetesExternalSecretsOperator's
`status.outputs.namespace`). The shared builder stamps that namespace into
every secretRef and ServiceAccount ref it renders. The modules create
credential Secrets BEFORE the CR and make the CR depend on them, so ESO
never observes a store whose secretRefs dangle.

## Namespace Fencing

`conditions` render to the CRD's namespace-selection conditions: exact
names, a label selector, or name regexes; entries union, and an empty list
means every namespace. This is the multi-tenancy fence for the cluster
grain — the alternative fence is the namespaced KubernetesSecretStore,
where scope itself is the boundary.
...
`version`), Go-duration patterns on intervals, and non-empty conditions —
each with a teaching message naming the fix.
