# The batch/v1 CronJob. Scheduling controls (schedule, time zone, concurrency,
# history retention) map at the top level; the Job stamped out at each run
# comes from spec.job_template. Every container (app and sidecars) renders
# through the same dynamic block from locals.all_containers, so semantics are
# identical for all container roles — mirroring the Pulumi module's shared
# builders.
#
# Identity is composed, not created: pods run as the ServiceAccount named in
# spec.job_template.pod.service_account. This module never creates
# ServiceAccounts, RBAC objects, certificates, gateways, or routes.
#
# CronJobs are mutable — the controller only reads the template when stamping
# out a run — so unlike Jobs, spec changes update in place.
...
resource "kubernetes_cron_job_v1" "this" {
  ...
  lifecycle {
    # The PARITY-EXCEPTION field below cannot be expressed through this
    ...
    precondition {
      condition     = try(var.spec.job_template.success_policy, null) == null
      error_message = "spec.job_template.success_policy is not supported on the Terraform engine for CronJobs (provider gap) — deploy with the Pulumi engine or omit success_policy."
    }
  }
  ...
  spec {
    ...
    # The provider names this argument `timezone` (the Kubernetes API field is
    # timeZone); emitted only when set so the cluster default applies otherwise.
    timezone = try(var.spec.time_zone, "") != "" && try(var.spec.time_zone, null) != null ? var.spec.time_zone : null

    starting_deadline_seconds = try(var.spec.starting_deadline_seconds, null)

    # Always emitted: "Forbid" is OUR default, deliberately safer than
    # upstream's "Allow" (see locals.concurrency_policy).
    concurrency_policy = local.concurrency_policy

    suspend = try(var.spec.suspend, false)

    successful_jobs_history_limit = try(var.spec.successful_jobs_history_limit, null)
    failed_jobs_history_limit     = try(var.spec.failed_jobs_history_limit, null)
    ...
    job_template {
      ...
      spec {
        ...
        # controller generates its own unique selector (controller-uid) for
        # each stamped-out Job and manages pod ownership through it — our
        # selector labels are for humans and tooling, not for the controller.

        # Batch controls: emitted only when the spec sets them; otherwise the
        # field is omitted and Kubernetes applies its own defaults
        # (parallelism 1, completions 1, backoffLimit 6, NonIndexed).
        parallelism             = try(var.spec.job_template.parallelism, null)
        completions             = try(var.spec.job_template.completions, null)
        completion_mode         = try(var.spec.job_template.completion_mode, "") != "" ? var.spec.job_template.completion_mode : null
        backoff_limit           = try(var.spec.job_template.backoff_limit, null)
        backoff_limit_per_index = try(var.spec.job_template.backoff_limit_per_index, null)
        max_failed_indexes      = try(var.spec.job_template.max_failed_indexes, null)
        active_deadline_seconds = try(var.spec.job_template.active_deadline_seconds, null)

        # The provider models this field as a string; the spec carries a number.
        ttl_seconds_after_finished = try(var.spec.job_template.ttl_seconds_after_finished, null) != null ? tostring(var.spec.job_template.ttl_seconds_after_finished) : null

        # PARITY-EXCEPTION: the Terraform kubernetes provider's job spec has no
        # `success_policy` block (the Pulumi module renders
              ...
              content {
                ...
                dynamic "on_pod_condition" {
                  ...
                  content {
                    ...
                    # Status defaults to "True" — the API requires the field,
                    # and "True" is the documented default in the spec.
        ...
        template {
          metadata {
            ...
            annotations = length(try(var.spec.job_template.pod.annotations, {})) > 0 ? var.spec.job_template.pod.annotations : null
          }
          ...
          spec {
            ...
            service_account_name             = try(var.spec.job_template.pod.service_account, "") != "" ? var.spec.job_template.pod.service_account : null
            automount_service_account_token  = try(var.spec.job_template.pod.automount_service_account_token, null)
            termination_grace_period_seconds = try(var.spec.job_template.pod.termination_grace_period_seconds, null)
            dns_policy                       = try(var.spec.job_template.pod.dns_policy, "") != "" ? var.spec.job_template.pod.dns_policy : null
            host_network                     = try(var.spec.job_template.pod.host_network, false)
            host_pid                         = try(var.spec.job_template.pod.host_pid, false)
            priority_class_name              = try(var.spec.job_template.pod.priority_class_name, "") != "" ? var.spec.job_template.pod.priority_class_name : null
            runtime_class_name               = try(var.spec.job_template.pod.runtime_class_name, "") != "" ? var.spec.job_template.pod.runtime_class_name : null
            node_selector                    = length(try(var.spec.job_template.pod.scheduling.node_selector, {})) > 0 ? var.spec.job_template.pod.scheduling.node_selector : null
            scheduler_name                   = try(var.spec.job_template.pod.scheduling.scheduler_name, "") != "" ? var.spec.job_template.pod.scheduling.scheduler_name : null
            ...
            dynamic "dns_config" {
              for_each = try(var.spec.job_template.pod.dns_config, null) != null ? [var.spec.job_template.pod.dns_config] : []
              content {
            ...
            dynamic "host_aliases" {
              for_each = try(var.spec.job_template.pod.host_aliases, [])
              content {
            ...
            dynamic "toleration" {
              for_each = try(var.spec.job_template.pod.scheduling.tolerations, [])
              content {
            ...
            dynamic "affinity" {
              for_each = (
                try(var.spec.job_template.pod.scheduling.node_affinity, null) != null ||
                try(var.spec.job_template.pod.scheduling.pod_affinity, null) != null ||
                try(var.spec.job_template.pod.scheduling.pod_anti_affinity, null) != null
              ) ? [var.spec.job_template.pod.scheduling] : []
              content {
            ...
            dynamic "topology_spread_constraint" {
              for_each = try(var.spec.job_template.pod.scheduling.topology_spread_constraints, [])
              content {
                ...
                # Empty match_labels self-spreads on the workload's own
                # selector — the overwhelmingly common intent, mirrored in the
                # Pulumi module.
                label_selector {
            ...
            dynamic "security_context" {
              for_each = try(var.spec.job_template.pod.security_context, null) != null ? [var.spec.job_template.pod.security_context] : []
              content {
            ...
            dynamic "container" {
              ...
              content {
                ...
                # Literal secret values reference the module-created env Secret
                # by the env var's own name; secretRef entries reference the
                # existing Secret directly.
                dynamic "env" {
                ...
                # PARITY-EXCEPTION: the Terraform kubernetes provider's
                # lifecycle handlers support exec/http_get/tcp_socket but NOT
                # the kubelet-native sleep action the Pulumi module renders. A
            ...
            # Pod volumes: union of every container's mounts, de-duplicated by
            # name (first declaration wins — see locals.pod_volumes).
            dynamic "volume" {
