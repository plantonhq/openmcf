...
func kubeletAddressTypes(spec *kubernetesmetricsserverv1.KubernetesMetricsServerSpec) string {
...
func metricResolution(spec *kubernetesmetricsserverv1.KubernetesMetricsServerSpec) string {
...
func tlsMap(tls *kubernetesmetricsserverv1.KubernetesMetricsServerTls) map[string]interface{} {
	if tls == nil {
		return nil
	}
	switch tls.GetType() {
	case kubernetesmetricsserverv1.KubernetesMetricsServerTlsType_helm:
		return map[string]interface{}{"type": "helm"}
	case kubernetesmetricsserverv1.KubernetesMetricsServerTlsType_cert_manager:
		out := map[string]interface{}{"type": "cert-manager"}
		if issuer := tls.GetCertManagerIssuer(); issuer != nil {
			kind := "Issuer"
			if issuer.GetKind() == kubernetesmetricsserverv1.KubernetesMetricsServerIssuerKind_cluster_issuer {
				kind = "ClusterIssuer"
			}
		...
		return out
	case kubernetesmetricsserverv1.KubernetesMetricsServerTlsType_existing_secret:
