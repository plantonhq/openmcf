# Provider requirements for the KubernetesGrpcRoute module.
#
# kubectl (alekc/kubectl) applies the GRPCRoute CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the GRPCRoute can be planned before the
