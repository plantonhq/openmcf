# KubernetesGrpcRoute Terraform Module

Creates a namespaced Kubernetes Gateway API `GRPCRoute` via the `kubectl_manifest`
...
```bash
planton tofu apply --manifest grpcroute.yaml
```
...
Existing GRPCRoutes can be adopted into state. `kubectl_manifest` uses the
...
| Output | Description |
|--------|-------------|
| `route_name` | Name of the created GRPCRoute (equals `metadata.name`) |
