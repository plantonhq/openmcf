...
- [KubernetesNamespace](/docs/catalog/kubernetes/kubernetesnamespace) — provides the target namespace; its resource profiles are the simpler alternative for T-shirt-size governance on namespaces Planton creates
- [KubernetesPriorityClass](/docs/catalog/kubernetes/kubernetespriorityclass) — defines the priority classes a `priority_class` scope selector budgets against
- [KubernetesNetworkPolicy](/docs/catalog/kubernetes/kubernetesnetworkpolicy) — the other per-namespace governance instrument: network segmentation alongside resource caps
