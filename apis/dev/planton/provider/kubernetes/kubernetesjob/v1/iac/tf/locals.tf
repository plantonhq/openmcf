...
#    controller adds its own controller-uid/job-name labels for pod ownership;
#    these labels are stamped on the pod template so `kubectl get pods -l` and
#    `kubectl logs -l` find this Job's pods without knowing controller
#    internals. There is deliberately NO version label — Jobs are one-shot, not
#    a deployment track.
#  - all_containers normalizes the app container (default name "app") and the
#    sidecars into ONE uniform list, so job.tf renders every container through
#    a single dynamic block — identical semantics for app and sidecars.
...
locals {
  ...
  restart_policy = try(coalesce(var.spec.restart_policy, "Never"), "Never")
