...
// mirrorMaker2MetricsRules is the canonical Strimzi JMX Prometheus
// Exporter rule set for Kafka Connect-protocol workers (MirrorMaker 2
// runs the Connect engine), copied VERBATIM from the pinned upstream
// example (examples/metrics/kafka-connect-metrics.yaml, ConfigMap key
// metrics-config.yml). The Terraform module embeds the SAME content
// (locals.mirrormaker2_metrics_rules) — the two must stay byte-identical
// so both engines render one ConfigMap shape.
const mirrorMaker2MetricsRules = `# Inspired by kafka-connect rules
