...
import (
	...
	kubernetesudproutev1alpha1 "github.com/plantonhq/planton/apis/dev/planton/provider/kubernetes/kubernetesudproute/v1alpha1"
	gatewayv1 "github.com/plantonhq/planton/pkg/kubernetes/kubernetestypes/gatewayapis/kubernetes/gateway/v1"
	"github.com/pulumi/pulumi/sdk/v3/go/pulumi"
)

// buildRules maps the Planton UDP route rules onto the typed crd2pulumi rules
// array. A UDP route rule carries only an optional name and the backend refs
// (no matches, no filters).
func buildRules(rules []*kubernetesudproutev1alpha1.KubernetesUdpRouteRule) gatewayv1.UDPRouteSpecRulesArray {
	arr := gatewayv1.UDPRouteSpecRulesArray{}
	for _, r := range rules {
		args := gatewayv1.UDPRouteSpecRulesArgs{}
...
// UDP routes have no per-backend filters.
//
...
func buildBackendRefs(backendRefs []*kubernetesapis.KubernetesGatewayApiBackendRef) gatewayv1.UDPRouteSpecRulesBackendRefsArray {
	arr := gatewayv1.UDPRouteSpecRulesBackendRefsArray{}
	for _, b := range backendRefs {
		args := gatewayv1.UDPRouteSpecRulesBackendRefsArgs{
