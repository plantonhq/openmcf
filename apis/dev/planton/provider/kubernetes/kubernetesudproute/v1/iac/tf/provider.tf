# Provider requirements for the KubernetesUdpRoute module.
#
# kubectl (alekc/kubectl) applies the UDPRoute CR: unlike the hashicorp
# kubernetes provider's kubernetes_manifest resource, kubectl_manifest needs
# no cluster connection at plan time, so the UDPRoute can be planned before the
