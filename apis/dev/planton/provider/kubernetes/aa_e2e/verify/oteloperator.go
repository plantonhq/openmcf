...
// otelCrds are the four CRDs the module stages from the pinned chart and
// applies OUTSIDE the release (retained on destroy — an operator
// uninstall never deletes the fleet's declarations). The feature-gated
// fifth CRD (clusterobservabilities) is deliberately not staged.
var otelCrds = []string{
...
// cert-issuance proof), all four opentelemetry.io CRDs established —
// and the TWO webhook proofs that only a live cluster can give:
//
...
//     hangs on.
//
// Destroy asserts the designed CRD keep: workloads gone, all four CRDs
// still present.
...
func (v *OtelOperatorVerifier) VerifyExists(ctx context.Context, kubeconfig string) error {
	...
	fmt.Printf("  [verify] operator rolled out and all 4 opentelemetry.io CRDs Established\n")
