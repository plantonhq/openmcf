...
For Planton users, the engine choice is transparent: applications use the same commands, the same client libraries, and the same data structures. The only user-visible difference is the `engine_version` field using `VALKEY_8_0` or `VALKEY_7_2` instead of `REDIS_7_0`.
...
- **Deletion protection**: Enable `deletion_protection_enabled: true` for all production instances. Prevents accidental destruction during `planton destroy` or `pulumi destroy`.
...
The Planton GcpMemorystoreInstance Pulumi module lives at:

```
apis/dev/planton/provider/gcp/gcpmemorystoreinstance/v1alpha1/iac/pulumi/
```
...
```
apis/dev/planton/provider/gcp/gcpmemorystoreinstance/v1alpha1/iac/tf/
```
