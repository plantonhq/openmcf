...
1. **Create README.md at v1 level**
   ...
   - **How:** Create a comprehensive README explaining the component's purpose, key features, use cases, and how to use it from Planton CLI
...
3. **Terraform module has no feature parity with Pulumi**
   ...
   - **Why it blocks:** Planton requires both Pulumi and Terraform implementations. The Terraform module is essentially non-existent
...
- Folder exists under correct provider hierarchy: `apis/dev/planton/provider/gcp/gcprouternat/v1/`
...
- `api.proto` exists (918 bytes > 500 bytes minimum)
  ...
  - Uses correct package: `dev.planton.provider.gcp.gcprouternat.v1`
...
```bash
cd apis/dev/planton/provider/gcp/gcprouternat/v1/
go test -v
```
...
> **Critical:** The Terraform module is essentially non-existent. This is a blocker for production use. Planton requires feature parity between Pulumi and Terraform implementations.
...
1. **Create README.md at v1 level**
   ...
   - **How:** Create comprehensive README explaining:
     ...
     - Benefits from Planton perspective
     - Getting started guidance
   - **Reference:** Look at `apis/dev/planton/provider/gcp/gcpvpcnetwork/v1/README.md` for structure
   - **Impact:** +6.67% completion score
...
2. **Create examples.md with comprehensive examples**
   ...
   - **How:** Add 4-5 working examples:
     ...
     - NAT with static external IPs
     - Complete configuration with all fields
     - NAT referencing existing GcpVpcNetwork resource
   - **Impact:** +6.66% completion score
