...
Deploys a Google Kubernetes Engine node pool — a group of identically configured Compute Engine VMs attached to a GKE cluster. This component is the compute companion to [GcpGkeCluster](/docs/catalog/gcp/gcpgkecluster): the cluster manages the control plane; node pools carry the workloads, each sized, tainted, and priced for its purpose.
...
- **A GKE cluster** — a [GcpGkeCluster](/docs/catalog/gcp/gcpgkecluster) resource (Standard mode; Autopilot clusters take no node pools)
- **A service account** ([GcpServiceAccount](/docs/catalog/gcp/gcpserviceaccount)) for production pools — the Compute Engine default SA is used otherwise
- **A Cloud KMS key** ([GcpKmsKey](/docs/catalog/gcp/gcpkmskey)) if using CMEK boot-disk encryption
...
- [GcpKmsKey](/docs/catalog/gcp/gcpkmskey) — CMEK key for boot-disk encryption
- [GcpGkeWorkloadIdentityBinding](/docs/catalog/gcp/gcpgkeworkloadidentitybinding) — grants workloads IAM identities
