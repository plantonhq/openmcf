...
OciKmsKey is an Planton component that deploys an OCI KMS encryption key inside a KMS vault. It provides a single declarative manifest to create a cryptographic key with a specific algorithm, key length, protection mode, and optional automatic rotation schedule.
...
- **Foreign key references** — `compartmentId` and `managementEndpoint` support `valueFrom` to reference Planton-managed OciCompartment and OciKmsVault resources.
