...
- **Automatic Tagging**: Standard Planton freeform tags applied to the container instance (resource kind, resource ID, organization, environment, and user-defined labels from metadata).
...
- **No Auto-Scaling.** OCI Container Instances do not have built-in auto-scaling. Each Planton resource creates exactly one container instance. For scale-out workloads, use OKE (OciContainerEngineCluster + OciContainerEngineNodePool) with horizontal pod autoscaling.
...
- **State Lifecycle Omitted**: The `state` field (ACTIVE/INACTIVE) is intentionally omitted from the spec. Planton resources are always deployed to their active state. Delete the resource to decommission the instance.
