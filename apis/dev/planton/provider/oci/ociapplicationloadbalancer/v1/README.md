...
The **OCI Load Balancer API Resource** provides a consistent and standardized interface for deploying and managing Oracle Cloud Infrastructure Application Load Balancers (Layer 7). This component bundles the load balancer with its backend sets, backends, listeners, certificates, hostnames, and rule sets into a single atomic deployment unit with the standard Planton KRM pattern. The spec defines 17 nested message types with 3 enums, covering the full configuration surface of OCI's Application Load Balancer API.

## Purpose

This API resource streamlines the deployment of OCI Application Load Balancers by wrapping 7 tightly coupled sub-resource types into one declarative manifest. An OCI load balancer requires at minimum the load balancer itself, one backend set with a health checker, and one listener — creating these as separate Planton components would require users to manage 3+ YAML manifests for even the simplest setup, with error-prone cross-references between them. The bundled approach enables users to:
...
- **Automatic Tagging**: Standard Planton freeform tags applied to the load balancer (resource kind, resource ID, organization, environment, and user-defined labels from metadata).
...
- **State Lifecycle Omitted**: The OCI load balancer `lifecycle_state` field is intentionally omitted. Planton resources are always deployed to their active state. Delete the resource to decommission the load balancer.
