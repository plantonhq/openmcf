...
OciObjectStorageBucket is an Planton component that deploys an OCI Object Storage bucket along with optional lifecycle policies, retention rules, and cross-region replication policies. It provides a single declarative manifest to manage all bucket-related resources as one unit.
...
- **Foreign key references** — `compartmentId` and `kmsKeyId` support `valueFrom` to reference other Planton-managed resources.
