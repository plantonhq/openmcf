...
OciMysqlDbSystem deploys an Oracle Cloud Infrastructure MySQL HeatWave Database System through Planton. It creates a single `oci_mysql_mysql_db_system` resource with configurable High Availability, automated backups, point-in-time recovery, read endpoints, encryption, and maintenance windows.
...
4. **Multi-resource composition** — reference Planton-managed compartments, subnets, and NSGs via `valueFrom` to compose a complete stack from independent manifests
