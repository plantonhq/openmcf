...
- [OciSubnet](/docs/catalog/oci/ocisubnet) — provides the subnets referenced by `subnetIds` via `valueFrom`
...
- [OciKmsKey](/docs/catalog/oci/ocikmskey) — provides signing keys for image verification via `valueFrom`
- [OciApiGateway](/docs/catalog/oci/ociapigateway) — exposes functions via HTTP endpoints using the `oracle_functions` backend type
