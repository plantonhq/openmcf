...
- **Provider Integration**  
  Seamlessly integrates with OpenMCF's Kubernetes provider configuration for secure cluster access.
...
3. **Namespace Management**  
   Creates the Kubernetes namespace if `create_namespace: true`, with appropriate OpenMCF labels.
...
- **Consistency**  
  Uses the same provider and namespace patterns as all other OpenMCF Kubernetes components.
