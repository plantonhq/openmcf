...
- Folder exists: `apis/org/openmcf/provider/kubernetes/kubernetesingressnginx/v1/`
...
```bash
cd apis/org/openmcf/provider/kubernetes/kubernetesingressnginx/v1/
go test -v
```
...
**Alternative:** Run the complete flow using `@forge-openmcf-component` to systematically address all gaps.
...
```bash
@audit-openmcf-component KubernetesIngressNginx
```
