...
```shell
cd apis/org/openmcf/provider/gcp/gcpsubnetwork/v1/iac/pulumi
```
...
```shell
# Preview
pulumi preview --config-file <(openmcf pulumi stack-input --manifest ../../hack/manifest.yaml)

# Deploy
pulumi up --config-file <(openmcf pulumi stack-input --manifest ../../hack/manifest.yaml) --yes

# Destroy
pulumi destroy --config-file <(openmcf pulumi stack-input --manifest ../../hack/manifest.yaml) --yes
```

#### Method 3: Using openmcf CLI (Simplest)
...
# Destroy
openmcf pulumi destroy --manifest hack/manifest.yaml
```
...
The script:
- Loads manifest using `openmcf` CLI
...
```shell
openmcf pulumi stack-input --manifest ../../hack/manifest.yaml > stack-input.json
```
...
on:
  push:
    branches: [main]
    paths:
      - 'apis/org/openmcf/provider/gcp/gcpsubnetwork/**'
...
- [OpenMCF CLI Documentation](../../../../../../../../../README.md)
