...
`GcpSubnetwork` is a OpenMCF resource for creating and managing custom-mode subnets in Google Cloud Platform VPC networks. It provides a simple, declarative interface for defining subnet configuration including primary and secondary IP ranges, regional placement, and Private Google Access settings.
...
# Destroy
openmcf pulumi destroy --manifest manifest.yaml
```
...
```shell
# Initialize
openmcf tofu init --manifest manifest.yaml --backend-type gcs \
