...
APIs), it streamlines the process of project creation within the OpenMCF multi-cloud deployment framework.
...
- **OpenMCF CLI**: Deploy and manage your new GCP project alongside other cloud resources using either Pulumi or
  Terraform under the hood.
- **Consistent Resource Model**: Rely on the same structure (apiVersion, kind, metadata, spec) that OpenMCF
...
- **Scalable Structure**: Expand your infrastructure with additional GCP resources—shared VPC networks, Cloud Storage
  buckets, GKE clusters—on top of the newly created project, within the same OpenMCF manifest.

## Example Usage

Below is a minimal YAML snippet demonstrating how to create a GCP project using OpenMCF. Notice that all fields
are
...
- **Using Pulumi**:
  ```bash
  openmcf pulumi up --manifest gcp_service_account.yaml --stack org/project/my-gcp-stack
  ```

- **Using Terraform**:
  ```bash
  openmcf terraform apply --manifest gcp_service_account.yaml --stack org/project/my-gcp-stack
  ```
