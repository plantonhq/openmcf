...
This component is part of OpenMCF's infrastructure-as-code framework, offering a simple protobuf-based API that generates production-ready Terraform and Pulumi code for GCP VPC deployment.
...
- **Multi-Cloud Consistency**: Part of OpenMCF's unified infrastructure API across GCP, AWS, and Azure
- **Infrastructure-as-Code Native**: Generates Terraform or Pulumi code with proper state management and drift detection
- **Foreign Key Support**: Reference other OpenMCF resources (like `GcpProject`) directly for clean dependency management
...
The `GcpVpc` resource follows OpenMCF's standard Kubernetes-style API structure:
