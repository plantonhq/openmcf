...
OpenMCF’s unified API resource model, letting you define your desired state using a single YAML manifest. Under
...
- **Kubernetes-Like Structure**  
  Uses `apiVersion`, `kind`, `metadata`, and `spec` fields for consistency across all OpenMCF resources.
...
1. **OpenMCF CLI**  
   Ensure you have [OpenMCF CLI](https://github.com/plantonhq/openmcf) installed. This command-line
...
2. **AWS Credentials**  
   Provide your AWS credentials in one of the following ways:
    ...
    - Create an AWS credential resource within OpenMCF and reference it in your manifest.
...
2. **Validate (Optional)**  
   ...
   ```bash
   openmcf validate --manifest awscertmanagercert.yaml
   ```

3. **Deploy**  
   Use the OpenMCF CLI to deploy via Pulumi or Terraform:
   ```bash
   # Pulumi example
   openmcf pulumi up --manifest awscertmanagercert.yaml --stack myorg/dev

   # Terraform example
   openmcf terraform apply --manifest awscertmanagercert.yaml --stack myorg/dev
   ```
...
Below are the key fields for the **AwsCertManagerCert** resource. Consult the Protobuf definitions in the OpenMCF
...
- **EC2 Launch Type**  
  If you prefer to run on EC2 instead of Fargate, the underlying OpenMCF modules can adapt accordingly by setting
...
- **[OpenMCF Guide](https://github.com/plantonhq/openmcf/blob/main/docs/Guide.md)**: Learn how to
