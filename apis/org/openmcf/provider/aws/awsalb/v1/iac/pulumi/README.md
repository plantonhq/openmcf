...
2. **Validate (Optional)**  
   ...
   ```bash
   openmcf validate --manifest awsalb.yaml
   ```
...
3. **Deploy**  
   ...
   ```bash
   # Pulumi example
   openmcf pulumi up --manifest awsalb.yaml --stack myorg/dev

   # Terraform example
   openmcf terraform apply --manifest awsalb.yaml --stack myorg/dev
   ```
...
Below are the key fields for the **AwsAlb** resource. Consult the Protobuf definitions in the OpenMCF
