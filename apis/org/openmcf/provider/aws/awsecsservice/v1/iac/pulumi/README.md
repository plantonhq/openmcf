...
2. **Validate (Optional)**  
   ...
   ```bash
   openmcf validate --manifest awsecsservice.yaml
   ```
...
3. **Deploy**  
   ...
   ```bash
   # Pulumi example
   openmcf pulumi up --manifest awsecsservice.yaml --stack myorg/dev

   # Terraform example
   openmcf terraform apply --manifest awsecsservice.yaml --stack myorg/dev
   ```
...
Below are the key fields for the **AwsEcsService** resource. Consult the Protobuf definitions in the OpenMCF
