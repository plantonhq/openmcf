...
- **OpenMCF CLI**: Deploy the same resource across multiple stacks using either Pulumi or Terraform under the
  hood.
- **Multi-Cloud Ready**: Combine AwsEcsService on AWS with other providers in the same manifest, adopting OpenMCF’s
...
check grace period using OpenMCF:
...
- **Using Pulumi**:
  ```bash
  openmcf pulumi up --manifest awsecsservice.yaml --stack org/project/my-stack
  ```
- **Using Terraform**:
  ```bash
  openmcf terraform apply --manifest awsecsservice.yaml --stack org/project/my-stack
  ```

OpenMCF will provision the ECS service, create or update the necessary IAM roles (if specified), assign the
