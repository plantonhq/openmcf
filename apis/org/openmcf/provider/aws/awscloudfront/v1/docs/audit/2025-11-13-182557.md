...
**Provider:** aws  
**Component Path:** `apis/org/openmcf/provider/aws/awscloudfront/v1/`  
**Enum Value:** 202  
**ID Prefix:** awscf
...
- Folder exists: `apis/org/openmcf/provider/aws/awscloudfront/v1/`
...
```bash
cd apis/org/openmcf/provider/aws/awscloudfront/v1/
go test -v
```
...
- Explains OpenMCF's opinionated approach and rationale
...
- **Strategic Insights:** Documents why OpenMCF's simplified 80/20 API design choices were made
...
1. **Create Pulumi Overview Document**
   ...
   - **How:** Document the module architecture (distribution.go, locals.go, outputs.go structure), explain how CloudFront-specific Pulumi resources are used, document any CloudFront-specific gotchas (us-east-1 certificate requirement), and explain the integration with OpenMCF's stack input pattern
...
**Path to reference:** `apis/org/openmcf/provider/aws/awsalb/v1/`
