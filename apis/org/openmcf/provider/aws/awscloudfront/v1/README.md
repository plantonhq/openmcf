...
This module can be provisioned with either Pulumi or Terraform via the OpenMCF CLI:

1. **Define Your Manifest**: Create a YAML file conforming to the `AwsCloudFront` API
2. **Validate**: `openmcf validate --manifest manifest.yaml`
3. **Deploy**: 
   - Pulumi: `openmcf pulumi up --manifest manifest.yaml --stack org/project/stack`
   - Terraform: `openmcf terraform apply --manifest manifest.yaml --stack org/project/stack`
