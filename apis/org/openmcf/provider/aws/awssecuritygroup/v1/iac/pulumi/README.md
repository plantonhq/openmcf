...
enforce least-privilege network policies within your VPC. This component is part of OpenMCF’s multi-cloud
...
- **Pulumi & Terraform Support**  
  Deploy using either Pulumi or Terraform by leveraging the OpenMCF CLI, which abstracts away the underlying
  differences, ensuring a consistent user experience.

- **Centralized Validation**  
  Validations defined in Protobuf (and enforced by OpenMCF) ensure you catch potential misconfigurations (e.g.,
...
1. **OpenMCF CLI**  
   Install the [OpenMCF CLI](https://github.com/plantonhq/openmcf) to manage manifests, validate
   configurations, and orchestrate deployments with Pulumi or Terraform.

2. **AWS Credentials**  
   Configure AWS credentials so OpenMCF can interact with AWS:
    - Environment variables `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`, or
    - A OpenMCF credential resource referencing your AWS account.
...
2. **Validate the Manifest**  
   ...
   ```bash
   openmcf validate --manifest security-group.yaml
   ```
...
3. **Deploy**  
   ...
   ```bash
   # Pulumi
   openmcf pulumi up --manifest security-group.yaml --stack myorg/dev

   # Terraform
   openmcf terraform apply --manifest security-group.yaml --stack myorg/dev
   ```
...
definitions in the OpenMCF repository.

### `apiVersion`

- **Description**: Must be `"aws.openmcf.org/v1"`.
- **Required**: Yes.
...
- **Provider Credential**  
  Override AWS credentials at run time using OpenMCF’s credential management or environment variables if needed.

- **Integration with Other Resources**  
  Combine your Security Group with ECS services, RDS instances, or any other OpenMCF resource in a single
  environment stack.
...
- **[OpenMCF Guide](https://github.com/plantonhq/openmcf/blob/main/docs/Guide.md)**: Explore
