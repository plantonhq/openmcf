...
Validate manifest:
```bash
openmcf validate --manifest ./lambda.yaml
```

Pulumi deploy:
```bash
openmcf pulumi update --manifest ./lambda.yaml --stack my-org/project/dev --module-dir apis/project/planton/provider/aws/awslambda/v1/iac/pulumi
```

Terraform deploy:
```bash
openmcf tofu apply --manifest ./lambda.yaml --auto-approve
```
