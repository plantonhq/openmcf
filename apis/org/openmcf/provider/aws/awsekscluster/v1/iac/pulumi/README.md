...
* Build the APIs and update the local cli
  - From the root of the `openmcf` repository, run:
    ```bash
...
* Update the `go.mod` file in the `aws-eks-cluster-pulumi-module` directory to point to the local `openmcf` repository.
  ```go
  replace github.com/plantonhq/openmcf => ../openmcf
  ```
...
* Run command
```bash
openmcf pulumi up --manifest <manifest_path>/eks.yaml  --stack <stack_path> --module-dir <path>/aws-eks-cluster-pulumi-module
```
* Output
```bash
