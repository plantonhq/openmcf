...
within the OpenMCF multi-cloud deployment framework.
...
- **Multi-Cloud Ready**: Combine AwsEksNodeGroup on AWS with other providers in the same manifest, adopting OpenMCF's
  uniform resource model.
- **Resource References**: Reference other OpenMCF resources (clusters, IAM roles, VPCs) using foreign key relationships.
...
Below is a minimal YAML snippet demonstrating how to configure and deploy an EKS node group using OpenMCF:
...
Once your YAML manifest is ready, you can deploy using OpenMCF's CLI. OpenMCF will validate the manifest
against the Protobuf schema and orchestrate everything in Pulumi or Terraform.

- **Using Pulumi**:
  ```bash
  openmcf pulumi up --manifest awseksnodegroup.yaml --stack org/project/my-stack
  ```
- **Using Terraform**:
  ```bash
  openmcf tofu apply --manifest awseksnodegroup.yaml --stack org/project/my-stack
  ```

OpenMCF will provision the EKS node group, configure the Auto Scaling Group, attach the IAM role, deploy nodes
