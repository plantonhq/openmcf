...
uniform resource that seamlessly integrates with the OpenMCF multi-cloud deployment framework.
...
- **OpenMCF CLI**: Manifests validated and deployed using the same familiar CLI, ensuring a consistent developer
  experience.
- **Multi-Cloud Ready**: Combine AWS ECS with other providers or resources in the same workflow, using the uniform
  OpenMCF resource model.
...
and exec auditing using OpenMCF:
...
You would then deploy this using the OpenMCF CLI (either Pulumi or Terraform under the hood):

```bash
openmcf pulumi up --manifest awsecscluster.yaml --stack org/project/stack
```

Or:

```bash
openmcf terraform apply --manifest awsecscluster.yaml --stack org/project/stack
```
...
via the OpenMCF community channels.
