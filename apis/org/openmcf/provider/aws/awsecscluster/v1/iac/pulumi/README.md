...
OpenMCF’s unified API resource model. It supports both Pulumi and Terraform under the hood, allowing teams to
...
1. **OpenMCF CLI**  
   Make sure you have [OpenMCF CLI](https://github.com/plantonhq/openmcf) installed and configured.
...
2. **AWS Credentials**  
   Ensure your AWS credentials are properly set up:
    ...
    - An AWS credential resource in OpenMCF, referencing your AWS credential via the manifest.
...
2. **Validate (Optional)**  
   Use the OpenMCF CLI to validate your manifest:
   ```bash
   openmcf validate --manifest awsecscluster.yaml
   ```
...
3. **Deploy**  
   ...
   ```bash
   # Pulumi example
   openmcf pulumi up --manifest awsecscluster.yaml --stack myorg/dev
   
   # Terraform example
   openmcf terraform apply --manifest awsecscluster.yaml --stack myorg/dev
   ```
...
- **Advanced Security**  
  Configure IAM roles or custom VPC networking directly within your ECS tasks through OpenMCF’s flexible YAML
  interface.
...
- **[OpenMCF Guide](https://github.com/plantonhq/openmcf/blob/main/docs/Guide.md)**: Learn about
