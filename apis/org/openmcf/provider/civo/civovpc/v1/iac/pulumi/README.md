...
- **CivoLabels**: Standard OpenMCF labels for resource tracking
...
1. Civo API credentials configured in OpenMCF
...
**Workaround:** Use OpenMCF metadata or resource labels for documentation.
...
This module is invoked by OpenMCF's orchestration layer:
...
The module applies standard OpenMCF labels for resource tracking:
...
**Note:** Civo Network resources don't currently support labels/tags via the provider. These labels are stored in OpenMCF metadata only.
...
```
github.com/plantonhq/openmcf/apis/org/openmcf/provider/civo/civovpc/v1
...
2. Ensure credentials exist in OpenMCF
