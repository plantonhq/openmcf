...
- **Tags**: The Civo Volume provider doesn't currently support tags. Tags specified in the spec are recorded in OpenMCF metadata but not applied to the Civo resource. Use Civo labels (automatically applied) for resource organization.
...
4. **Tagging Strategy**: While tags aren't applied to Civo resources, use them in the spec for OpenMCF metadata tracking, cost allocation, and organizational purposes.
