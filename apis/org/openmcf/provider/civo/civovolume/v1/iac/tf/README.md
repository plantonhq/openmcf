...
- **Labels Integration**: Automatically applies OpenMCF labels for resource tracking
...
**Note**: OpenMCF labels (in `locals.tf`) are tracked internally but also not applied to the Civo resource, as the provider doesn't support labels/tags.
...
- **OpenMCF**: [github.com/plantonhq/openmcf/issues](https://github.com/plantonhq/openmcf/issues)
